...
public:
    ...
    [[nodiscard]] auto get_account_by_username(std::string_view username)
    ...
    [[nodiscard]] auto create_character(account_id id, const character_create_info& info)
        -> result<player_id, auth_error>;
    [[nodiscard]] auto delete_character(account_id id, player_id char_id)
        -> result<void, auth_error>;
    [[nodiscard]] auto get_character(player_id char_id) -> result<character_summary, auth_error>;
    [[nodiscard]] auto load_character_full(player_id char_id, account_id owner)
        -> result<character_full_data, auth_error>;
    [[nodiscard]] auto save_character(const character_full_data& data)
        -> result<void, auth_error>;

    // Ban management
    void ban_account(account_id id, std::string_view reason,
...
private:
    ...
    [[nodiscard]] auto db_get_account_by_username(std::string_view username)
        -> result<account, auth_error>;
    [[nodiscard]] auto db_create_account(std::string_view username, std::string_view password_hash)
        -> result<account_id, auth_error>;
