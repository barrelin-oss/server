...
auto war_system::schedule_war(war_type type, std::chrono::system_clock::time_point start_time)
    -> result<war_id, war_result_code> {
...
auto war_system::start_war(war_type type) -> result<war_id, war_result_code> {
    ...
    war.ends_at = war.started_at +
        std::chrono::seconds(war.config.preparation_seconds + war.config.duration_seconds);
...
auto war_system::end_war(war_id id) -> war_result_code {
    auto* war = get_war(id);
    if (!war) return war_result_code::war_not_found;
...
auto war_system::join_war(war_id wid, player_id player, const std::string& player_name, war_faction faction)
    -> war_result_code {
    ...
    LOG_DEBUG(general, "Player {} joined war {} as faction {}",
        player.value, wid.value, static_cast<int>(faction));
...
auto war_system::leave_war(war_id wid, player_id player) -> war_result_code {
    ...
    if (!participant) return war_result_code::not_participant;
...
auto war_system::respawn_participant(war_id wid, player_id player) -> bool {
    auto* war = get_war(wid);
    if (!war || !war->is_active()) return false;

    auto* participant = war->get_participant(player);
    if (!participant) return false;

    if (participant->status != participant_status::dead) return false;
...
void war_system::record_kill(war_id wid, player_id killer, player_id victim) {
    ...
    if (victim_participant) {
        ...
        victim_participant->respawn_timer_seconds = war->config.base_respawn_seconds +
            (victim_participant->deaths * war->config.respawn_increment_per_death);
...
void war_system::update_objective_capture(war_id wid, uint16_t objective_id, war_faction capturing, int32_t progress) {
    ...
    if (!obj || !obj->can_be_captured) return;
    ...
    if (obj->capture_progress >= 100) {
        ...
        LOG_DEBUG(general, "Objective {} captured by faction {} in war {}",
            objective_id, static_cast<int>(capturing), wid.value);
    }
}

auto war_system::get_objective(war_id wid, uint16_t objective_id) -> war_objective* {
    auto* war = get_war(wid);
    if (!war) return nullptr;
...
auto war_system::calculate_rewards(war_id wid, player_id player) const -> war_rewards {
    ...
    if (!war) return rewards;

    const auto* participant = war->get_participant(player);
    if (!participant) return rewards;
...
void war_system::transition_war_state(war_state& war, war_status new_state) {
    ...
    LOG_INFO(general, "War {} transitioned from state {} to {}",
        war.id.value, static_cast<int>(old_state), static_cast<int>(new_state));
...
void war_system::finalize_war(war_state& war) {
    ...
    LOG_INFO(general, "War {} ended. Winner: faction {}",
        war.id.value, static_cast<int>(winner));
}
