...
void mark_fully_migrated(protocol::message_id msg_id)
{
    ...
    LOG_INFO(proto_bridge, "Message {} marked as fully migrated", protocol::message_id_string(msg_id));
}
...
{
    std::lock_guard lock(g_migrated_mutex);
    return std::vector<protocol::message_id>(g_fully_migrated.begin(), g_fully_migrated.end());
}
...
void log_registered_handlers()
{
    ...
    LOG_INFO(proto_bridge,
             "=== {} handlers registered, {:.1f}% migration progress ===",
             handlers.size(),
             migration_progress());
}
...
void log_handler_stats()
{
    ...
    LOG_INFO(proto_bridge,
             "  Total proc time:   {}ms",
             std::chrono::duration_cast<std::chrono::milliseconds>(global.total_processing_time).count());
    ...
    if (!msg_stats.empty())
    {
        ...
        // Sort by total count
        std::sort(msg_stats.begin(),
                  msg_stats.end(),
                  [](const auto& a, const auto& b) { return a.total_count > b.total_count; });
        ...
        for (const auto& stats : msg_stats)
        {
            if (count++ >= 10)
                break;
            LOG_INFO(proto_bridge,
                     "  {}: {} total, {} success, {} fallback, {} errors, max {}us",
                     protocol::message_id_string(stats.msg_id),
                     stats.total_count,
                     stats.success_count,
                     stats.fallback_count,
                     stats.error_count,
                     std::chrono::duration_cast<std::chrono::microseconds>(stats.max_time).count());
        }
    }
