...
query_result::query_result(pqxx::result result) : result_(std::move(result)) {}
...
void database_system::initialize()
{
    ...
    pool_config pool_cfg{.host = config_.host,
                         .port = config_.port,
...
auto database_system::execute_unsafe(std::string_view query) -> hb::result<query_result, std::string>
{
...
auto database_system::execute(std::string_view query) -> hb::result<query_result, std::string>
{
...
auto database_system::execute_transaction(transaction_func func) -> hb::result<void, std::string>
{
    PERF_TIMER(perf_stats_, perf::metric_category::db_query);
...
auto database_system::prepare(std::string_view name, std::string_view query) -> hb::result<void, std::string>
{
    auto conn = pool_.acquire();
    if (conn.is_err())
    {
