...
struct war_objective
{
    ...
    [[nodiscard]] auto is_captured() const -> bool { return capture_progress >= 100; }

    [[nodiscard]] auto is_contested() const -> bool
    {
        return capturing_faction != war_faction::neutral && capturing_faction != controlling_faction;
    }
};
...
struct war_rewards
{
    ...
    {
        return experience > 0 || gold > 0 || contribution_points > 0 || honor_points > 0 || !items.empty();
    }
};
...
namespace std
{
template<> struct hash<hb::war::war_id>
{
    auto operator()(const hb::war::war_id& id) const noexcept -> size_t { return hash<uint32_t>{}(id.value); }
};

template<> struct hash<hb::war::territory_id>
{
    auto operator()(const hb::war::territory_id& id) const noexcept -> size_t { return hash<uint16_t>{}(id.value); }
};
} // namespace std
