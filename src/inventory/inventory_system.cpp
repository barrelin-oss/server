...
void inventory_system::create_inventory(entity_id owner) {
    if (inventories_.contains(owner)) return;
...
void inventory_system::create_bank(entity_id owner) {
    if (banks_.contains(owner)) return;
...
auto inventory_system::deposit_item(entity_id owner, int16_t inv_slot)
    -> inventory_result
{
    auto* inv = get_inventory(owner);
    auto* bank = get_bank(owner);

    if (!inv || !bank) return inventory_result::failed;
...
auto inventory_system::withdraw_item(entity_id owner, int16_t bank_slot)
    -> inventory_result
{
    auto* inv = get_inventory(owner);
    auto* bank = get_bank(owner);
...
auto inventory_system::remove_gold(entity_id owner, int64_t amount) -> bool {
    if (amount < 0) return false;
    if (!has_gold(owner, amount)) return false;
...
void inventory_system::cancel_trade(entity_id player) {
    ...
    if (partner_it == trade_partners_.end()) return;
...
auto inventory_system::add_to_trade(entity_id player, item_id item)
    -> inventory_result
{
    auto* window = get_trade_window(player);
    if (!window) return inventory_result::failed;
    if (window->locked) return inventory_result::failed;
...
auto inventory_system::remove_from_trade(entity_id player, item_id item)
    -> inventory_result
{
    auto* window = get_trade_window(player);
    if (!window) return inventory_result::failed;
...
void inventory_system::set_trade_gold(entity_id player, int32_t amount) {
    auto* window = get_trade_window(player);
    if (!window || window->locked) return;
...
auto inventory_system::complete_trade(entity_id player1, entity_id player2) -> bool {
    ...
    if (!window1->confirmed || !window2->confirmed) return false;
    if (!window1->locked || !window2->locked) return false;

    // Verify gold
    if (!has_gold(player1, window1->gold_offered)) return false;
    if (!has_gold(player2, window2->gold_offered)) return false;
