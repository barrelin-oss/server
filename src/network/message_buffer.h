...
public:
    explicit message_writer(size_t initial_capacity = 256) {
    ...
    [[nodiscard]] auto take() -> std::vector<uint8_t> {
...
public:
    ...
    explicit receive_buffer(size_t capacity = default_capacity) {
    ...
    [[nodiscard]] auto has_complete_message() const -> bool {
        ...
        uint16_t length = static_cast<uint16_t>(buffer_[0]) |
    ...
    [[nodiscard]] auto extract_message() -> std::vector<uint8_t> {
        ...
        std::vector<uint8_t> message(buffer_.begin() + 2,
