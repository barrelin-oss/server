...
void map::initialize(map_id id, const map_config& config)
{
    ...
    LOG_DEBUG(general, "Map {} initialized: {}x{} ({} tiles)", config_.name, config_.width, config_.height, total);
}
...
auto map::load_from_file(const std::filesystem::path& path) -> result<void, std::string>
{
    ...
    auto parse_value = [&](const std::string& key) -> int16_t
    {
        ...
        while (pos < header_str.length() &&
               (header_str[pos] == ' ' || header_str[pos] == '=' || header_str[pos] == '\t'))
        {
            ++pos;
        }
        if (pos >= header_str.length())
            return 0;
    ...
    LOG_DEBUG(general, "Loaded map {}: {}x{}, tile_size={}, {} tiles", config_.name, size_x, size_y, tile_size, total);
...
auto map::remove_teleport(const position& pos) -> bool
{
    ...
    if (is_valid_position(pos))
    {
        auto& tile = static_tiles_[tile_index(pos)];
        tile.flags =
            static_cast<tile_flags>(static_cast<uint8_t>(tile.flags) & ~static_cast<uint8_t>(tile_flags::is_teleport));
    }
    ...
    try
    {
        ...
        if (root["random_mob_generator"])
        {
            ...
            LOG_DEBUG(general,
                      ...
                      config_.name,
                      config_.random_mob_generator.enabled,
                      config_.random_mob_generator.level);
        }
        ...
        if (root["initial_points"])
        {
            ...
            {
                initial_point ip{.id = static_cast<int16_t>(node["id"].as<int>()),
        ...
        if (root["safe_zones"])
        {
            ...
            {
                safe_zone sz{.area = rect{static_cast<int16_t>(node["left"].as<int>()),
        ...
        if (root["spawners"])
        {
            ...
            {
                spot_mob_generator smg{.id = static_cast<int16_t>(node["id"].as<int>()),
                                       .type = static_cast<int16_t>(node["type"].as<int>()),
                                       .area = rect{static_cast<int16_t>(node["x1"].as<int>()),
                                       ...
                                       .elite_chance = static_cast<int16_t>(node["elite_chance"] ? node["elite_chance"].as<int>() : 0),
                                       .enabled = true};
        ...
        if (root["waypoints"])
        {
            ...
            {
                waypoint wp{.id = static_cast<int16_t>(node["id"].as<int>()),
                            .x = static_cast<int16_t>(node["x"].as<int>()),
                            .y = static_cast<int16_t>(node["y"].as<int>())};
                waypoints_[wp.id] = wp;
            }
        ...
        if (root["mineral_points"])
        {
            ...
            {
                mineral_point mp{.id = static_cast<int16_t>(node["id"].as<int>()),
                                 .x = static_cast<int16_t>(node["x"].as<int>()),
                                 .y = static_cast<int16_t>(node["y"].as<int>())};
        ...
        if (root["fish_points"])
        {
            ...
            {
                fish_point fp{.id = static_cast<int16_t>(node["id"].as<int>()),
                              .x = static_cast<int16_t>(node["x"].as<int>()),
                              .y = static_cast<int16_t>(node["y"].as<int>())};
        ...
        if (root["teleports"])
        {
            ...
            {
                position src{static_cast<int16_t>(node["src_x"].as<int>()),
                             static_cast<int16_t>(node["src_y"].as<int>())};
                teleport_dest dest{.dest_map = node["dest_map"].as<std::string>(),
        ...
        LOG_INFO(general,
                 "Map {} config loaded (YAML): {} initial points, {} safe zones, {} spawners, {} teleports, {} mineral "
                 "points, {} fish points",
                 config_.name,
                 initial_points_.size(),
                 safe_zones_.size(),
                 mob_spawners_.size(),
                 teleports_.size(),
                 mineral_points_.size(),
                 fish_points_.size());
