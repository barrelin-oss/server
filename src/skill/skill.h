...
{
    if (skill_level >= 120)
        return mastery_level::grand_master;
    if (skill_level >= 100)
        return mastery_level::master;
    if (skill_level >= 80)
        return mastery_level::expert;
    if (skill_level >= 60)
        return mastery_level::journeyman;
    if (skill_level >= 40)
        return mastery_level::apprentice;
    if (skill_level >= 20)
        return mastery_level::beginner;
...
struct player_skills
{
    ...
    [[nodiscard]] auto get(skill_type type) -> skill_state& { return skills[static_cast<size_t>(type)]; }

    [[nodiscard]] auto get(skill_type type) const -> const skill_state& { return skills[static_cast<size_t>(type)]; }

    [[nodiscard]] auto level(skill_type type) const -> int16_t { return get(type).level; }

    void set_level(skill_type type, int16_t lvl) { get(type).level = lvl; }

    // Get combat skill for a weapon type
    [[nodiscard]] auto get_weapon_skill(skill_type weapon) const -> int16_t { return get(weapon).level; }
