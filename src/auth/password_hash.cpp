...
auto hash_password(std::string_view password,
                   const password_hash_config& config) -> hb::result<std::string, std::string>
{
    if (!init_sodium())
    {
    ...
    int hash_result = crypto_pwhash_str(hash, password.data(), password.size(), config.ops_limit, config.memory_limit);
...
auto verify_password(std::string_view password, std::string_view hash) -> bool
{
    ...
    int verify_result = crypto_pwhash_str_verify(hash_str.c_str(), password.data(), password.size());
...
auto needs_rehash(std::string_view hash) -> bool
{
    ...
    return crypto_pwhash_str_needs_rehash(
               hash_str.c_str(), crypto_pwhash_OPSLIMIT_INTERACTIVE, crypto_pwhash_MEMLIMIT_INTERACTIVE) != 0;
}
