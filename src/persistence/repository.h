...
template<typename T, typename Id, typename Hash = std::hash<Id>> class memory_repository : public repository<T, Id>
{
public:
    using entity_type = T;
    using id_type = Id;
    ...
    explicit memory_repository(id_extractor get_id) : get_id_(std::move(get_id)) {}
    ...
    auto exists(Id id) -> bool override { return data_.contains(id); }
    ...
    auto count() -> size_t override { return data_.size(); }
