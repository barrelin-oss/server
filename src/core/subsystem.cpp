...
subsystem_manager::subsystem_manager()
    : event_bus_(std::make_unique<hb::event_bus>())
{
}
    ...
    for (auto& subsystem : subsystems_) {
        if (!subsystem->is_initialized()) {
            ...
            try {
                ...
                LOG_ERROR(general, "Failed to initialize subsystem {}: {}",
                    subsystem->name(), e.what());
                throw;
            }
        }
    }
    ...
    for (auto it = subsystems_.rbegin(); it != subsystems_.rend(); ++it) {
        ...
        if (subsystem->is_initialized()) {
            ...
            try {
                ...
                LOG_ERROR(general, "Error shutting down subsystem {}: {}",
                    subsystem->name(), e.what());
            }
        }
    }
