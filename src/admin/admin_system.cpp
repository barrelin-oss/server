...
auto admin_system::has_command(std::string_view name) const -> bool {
    ...
    if (commands_.contains(name_lower)) return true;
    if (aliases_.contains(name_lower)) return true;
    return false;
}
...
auto admin_system::execute(player_id executor, std::string_view command_string) -> command_result {
    ...
    if (admin_info_it != admins_.end()) {
        ...
        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
            now - admin_info_it->second.last_command).count();
...
auto admin_system::can_execute(player_id executor, std::string_view command_name) const -> bool {
    ...
    if (admin_it == admins_.end()) return false;
...
void admin_system::set_admin_level(player_id player, admin_level level, player_id changed_by) {
    ...
    LOG_INFO(admin, "Player {} admin level changed from {} to {} by {}",
             player.value, to_string(old_level), to_string(level), changed_by.value);
...
auto admin_system::ban_player(player_id target, player_id banned_by, std::string_view reason,
    ...
    LOG_INFO(admin, "Player {} banned by {} for: {} (duration: {}s)",
             target.value, banned_by.value, reason, duration_seconds);
...
auto admin_system::mute_player(player_id target, player_id muted_by, std::string_view reason,
                                int64_t duration_seconds) -> command_result {
    ...
    LOG_INFO(admin, "Player {} muted by {} for: {} (duration: {}s)",
             target.value, muted_by.value, reason, duration_seconds);
...
auto admin_system::get_mute_remaining(player_id player) const -> int64_t {
    ...
    if (it == muted_players_.end()) return 0;

    if (it->second == 0) return -1;  // Permanent

    int64_t now = std::time(nullptr);
    if (it->second <= now) return 0;
...
auto admin_system::get_log_for_player(player_id executor, size_t count) const
...
auto admin_system::get_help_all(admin_level level) const -> std::string {
    ...
    std::sort(commands.begin(), commands.end(),
              [](const command_info* a, const command_info* b) {
...
void admin_system::register_builtin_commands() {
    // Help command
    {
        ...
        register_command(info, [this](const command_context& ctx) -> command_result {
            if (!ctx.args.empty()) {
    ...
    // Mute command
    {
        ...
        info.arguments = {
            make_arg("player_id", arg_type::integer, true, "", "Player ID to mute"),
    ...
    // Kick command
    {
        ...
        info.required_level = admin_level::moderator;
        info.arguments = {
            make_arg("player_id", arg_type::integer, true, "", "Player ID to kick"),
    ...
    // Ban command
    {
        ...
        info.arguments = {
            make_arg("player_id", arg_type::integer, true, "", "Player ID to ban"),
            make_arg("duration", arg_type::integer, false, "0", "Duration in seconds (0=permanent)"),
    ...
    // Announce command
    {
        ...
        register_command(info, [](const command_context& ctx) -> command_result {
            if (ctx.args.empty()) {
...
void admin_system::log_command(const command_log_entry& entry) {
    ...
    LOG_DEBUG(admin, "[{}] {}: {} - {}",
              to_string(entry.executor_level), entry.executor_name,
              entry.full_command, entry.success ? "OK" : "FAILED");
}
