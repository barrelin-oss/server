...
auto serialize_rank_configs(const std::array<guild_rank_config, guild::max_ranks>& ranks) -> std::string
{
    ...
    {
        arr.push_back({{"name", r.name}, {"permissions", static_cast<uint32_t>(r.permissions)}});
    }
    return arr.dump();
}
    ...
    try
    {
        ...
        for (size_t i = 0; i < arr.size() && i < guild::max_ranks; ++i)
        {
            ...
            if (obj.contains("name"))
                ranks[i].name = obj["name"].get<std::string>();
            if (obj.contains("permissions"))
                ranks[i].permissions = static_cast<guild_permission>(obj["permissions"].get<uint32_t>());
        }
    }
...
auto social_system::load_guilds_from_database() -> result<void, std::string>
{
    ...
    // Load all guilds
    auto guilds_result = database_->execute_params("SELECT id, name, tag, motd, nation, leader_id, level, experience, "
    ...
    auto members_result =
        database_->execute_params("SELECT gm.guild_id, gm.character_id, gm.rank, gm.contribution, gm.note, c.name "
...
void social_system::connect_guild_member(player_id character_id, player_id runtime_id, const std::string& name)
{
    ...
    LOG_DEBUG(general,
              "Guild member '{}' connected (char_id={}, runtime_id={}, guild='{}')",
              name,
              character_id.value,
              runtime_id.value,
              g->name);
...
auto social_system::insert_guild_to_database(const guild& g) -> result<void, std::string>
{
    ...
    return database_->execute_transaction(
        [&](pqxx::work& txn) -> result<void, std::string>
        {
            txn.exec_params("INSERT INTO guilds (id, name, tag, motd, nation, leader_id, level, experience, "
            ...
            for (const auto& m : g.members)
            {
                if (!m.character_id.is_valid())
                    continue;
                txn.exec_params("INSERT INTO guild_members (guild_id, character_id, rank, contribution, note) "
                                "VALUES ($1, $2, $3, $4, $5)",
                                static_cast<int>(g.id.value),
...
auto social_system::save_guild_to_database(guild_id gid) -> result<void, std::string>
{
    ...
    auto* g = get_guild(gid);
    if (!g)
        return result<void, std::string>::err("Guild not found");
    ...
    return database_->execute_transaction(
        [&](pqxx::work& txn) -> result<void, std::string>
        {
            txn.exec_params("UPDATE guilds SET tag=$1, motd=$2, nation=$3, leader_id=$4, level=$5, "
            ...
            txn.exec_params("DELETE FROM guild_members WHERE guild_id=$1", static_cast<int>(gid.value));
...
auto social_system::delete_guild_from_database(guild_id gid) -> result<void, std::string>
{
    ...
    auto res = database_->execute_params("DELETE FROM guilds WHERE id=$1", static_cast<int>(gid.value));
    if (res.is_err())
        return result<void, std::string>::err(res.error());
    return result<void, std::string>::ok();
}
...
auto social_system::create_guild(player_id founder,
                                 const std::string& name,
                                 const std::string& tag) -> result<guild_id, guild_result>
{
...
auto social_system::disband_guild(player_id player, guild_id gid) -> guild_result
{
    ...
    for (const auto& member : g->members)
    {
        ...
        if (member.character_id.is_valid())
            character_guild_index_.erase(member.character_id);
    }
...
auto social_system::invite_to_guild(player_id inviter, guild_id gid, player_id invitee) -> guild_result
{
    ...
    pending_guild_invites_[invitee] =
        pending_guild_invite{.guild = gid,
                             .inviter = inviter,
                             .guild_name = g->name,
                             .guild_tag = g->tag,
    ...
    LOG_DEBUG(
        general, "Player {} invited player {} to guild {} ({})", inviter.value, invitee.value, g->name, gid.value);
...
{
    if (kicker == target)
        return guild_result::cannot_kick_self;
...
auto social_system::admin_kick_from_guild(guild_id gid, std::string_view target_name) -> guild_result
{
    ...
    // Find member by name
    auto it = std::find_if(
        g->members.begin(), g->members.end(), [&](const guild_member& m) { return m.name == target_name; });
    if (it == g->members.end())
        return guild_result::player_not_member;

    if (it->player.is_valid())
        player_guilds_.erase(it->player);
    if (it->character_id.is_valid())
        character_guild_index_.erase(it->character_id);
    g->members.erase(it);
...
auto social_system::disband_party(player_id leader) -> party_result
{
    ...
    if (!p->is_leader(leader))
        return party_result::not_leader;
...
auto social_system::invite_to_party(player_id inviter, player_id invitee) -> party_result
{
    ...
    if (!p->is_leader(inviter))
        return party_result::not_leader;
    if (p->is_full())
        return party_result::party_full;
    if (get_player_party(invitee).is_valid())
    ...
    pending_party_invites_[invitee].push_back(
        party_invite{pid, inviter, invitee, std::chrono::steady_clock::now() + party::invite_duration});
...
{
    if (kicker == target)
        return party_result::cannot_kick_self;
    ...
    if (!p->is_leader(kicker))
        return party_result::not_leader;
    if (!p->is_member(target))
        return party_result::player_not_member;
...
auto social_system::set_party_leader(player_id current_leader, player_id new_leader) -> party_result
{
    ...
    if (!p->is_leader(current_leader))
        return party_result::not_leader;
    if (!p->is_member(new_leader))
        return party_result::player_not_member;
...
auto social_system::has_party_invite(player_id player) const -> bool
{
    ...
    if (it == pending_party_invites_.end())
        return false;
...
auto social_system::load_friends_from_database() -> result<void, std::string>
{
    ...
    auto req_result = database_->execute_params("SELECT fr.requester_id, fr.requestee_id, fr.created_at, "
    ...
    // Load blocks
    auto blocks_result = database_->execute_params("SELECT blocker_id, blocked_id FROM friend_blocks");
    ...
    LOG_INFO(general,
             "Loaded friend data: {} requests, {} friendships, {} blocks",
             req_rows.size(),
             friends_rows.size(),
             blocks_rows.size());
    return result<void, std::string>::ok();
...
void social_system::connect_friend(player_id character_id, player_id runtime_id, const std::string& name)
{
    ...
    LOG_DEBUG(general,
              "Friend system: '{}' connected (char_id={}, runtime_id={})",
              name,
              character_id.value,
              runtime_id.value);
}
...
auto social_system::send_friend_request(player_id requester_char_id, player_id target_char_id) -> friend_result
{
    ...
    {
        auto name_res = database_->execute_params("SELECT name FROM characters WHERE id = $1",
...
auto social_system::accept_friend_request(player_id accepter_char_id, player_id requester_char_id) -> friend_result
{
    ...
    LOG_DEBUG(
        general, "Friend request accepted: char {} accepted char {}", accepter_char_id.value, requester_char_id.value);
    return friend_result::success;
}
...
auto social_system::cancel_friend_request(player_id requester_char_id, player_id target_char_id) -> friend_result
{
    ...
    LOG_DEBUG(general,
              ...
              requester_char_id.value,
              target_char_id.value);
    return friend_result::success;
}
...
auto social_system::are_friends(player_id char_a, player_id char_b) const -> bool
{
    ...
    if (it == character_friends_.end())
        return false;
    for (const auto& f : it->second)
    {
        if (f.character_id == char_b)
            return true;
    }
    return false;
}
...
auto social_system::has_pending_request(player_id requester_char, player_id target_char) const -> bool
{
    ...
    if (it == outgoing_requests_.end())
        return false;
    for (const auto& r : it->second)
    {
        if (r.requestee_char_id == target_char)
            return true;
    }
    return false;
}
...
auto social_system::lookup_character_by_name(const std::string& name) -> std::pair<player_id, std::string>
{
    ...
    {
        auto res =
            database_->execute_params("SELECT id, name FROM characters WHERE LOWER(name) = LOWER($1) LIMIT 1", name);
...
auto social_system::lookup_character_name(player_id character_id) -> std::string
{
    ...
    if (database_)
    {
        auto res = database_->execute_params("SELECT name FROM characters WHERE id = $1",
                                             static_cast<int>(character_id.value));
        if (res.is_ok() && res.value().size() > 0)
        {
...
auto social_system::save_friend_request_db(player_id requester_char,
                                           player_id requestee_char) -> result<void, std::string>
{
    if (!database_)
        return result<void, std::string>::ok();
    auto res = database_->execute_params("INSERT INTO friend_requests (requester_id, requestee_id) VALUES ($1, $2) "
                                         "ON CONFLICT (requester_id, requestee_id) DO NOTHING",
                                         static_cast<int>(requester_char.value),
                                         static_cast<int>(requestee_char.value));
    if (res.is_err())
        return result<void, std::string>::err(res.error());
    return result<void, std::string>::ok();
}

auto social_system::delete_friend_request_db(player_id requester_char,
                                             player_id requestee_char) -> result<void, std::string>
{
    if (!database_)
        return result<void, std::string>::ok();
    auto res = database_->execute_params("DELETE FROM friend_requests WHERE requester_id = $1 AND requestee_id = $2",
                                         static_cast<int>(requester_char.value),
                                         static_cast<int>(requestee_char.value));
    if (res.is_err())
        return result<void, std::string>::err(res.error());
...
auto social_system::save_friend_relationship_db(player_id char_a, player_id char_b) -> result<void, std::string>
{
    ...
    auto res = database_->execute_params("INSERT INTO friends (character_a, character_b) VALUES ($1, $2) "
                                         "ON CONFLICT (character_a, character_b) DO NOTHING",
                                         static_cast<int>(lower),
                                         static_cast<int>(higher));
    if (res.is_err())
...
auto social_system::delete_friend_relationship_db(player_id char_a, player_id char_b) -> result<void, std::string>
{
    if (!database_)
        return result<void, std::string>::ok();
    auto [lower, higher] = std::minmax(char_a.value, char_b.value);
    auto res = database_->execute_params("DELETE FROM friends WHERE character_a = $1 AND character_b = $2",
                                         static_cast<int>(lower),
                                         static_cast<int>(higher));
    if (res.is_err())
        return result<void, std::string>::err(res.error());
...
auto social_system::save_block_db(player_id blocker_char, player_id blocked_char) -> result<void, std::string>
{
    if (!database_)
        return result<void, std::string>::ok();
    auto res = database_->execute_params("INSERT INTO friend_blocks (blocker_id, blocked_id) VALUES ($1, $2) "
                                         "ON CONFLICT (blocker_id, blocked_id) DO NOTHING",
                                         static_cast<int>(blocker_char.value),
                                         static_cast<int>(blocked_char.value));
    if (res.is_err())
        return result<void, std::string>::err(res.error());
    return result<void, std::string>::ok();
}
...
auto social_system::delete_block_db(player_id blocker_char, player_id blocked_char) -> result<void, std::string>
{
    if (!database_)
        return result<void, std::string>::ok();
    auto res = database_->execute_params("DELETE FROM friend_blocks WHERE blocker_id = $1 AND blocked_id = $2",
                                         static_cast<int>(blocker_char.value),
                                         static_cast<int>(blocked_char.value));
    if (res.is_err())
        return result<void, std::string>::err(res.error());
