...
public:
    ...
    explicit message_reader(std::span<const uint8_t> data)
        : data_(data)
        , pos_(0)
    {}

    // Construct from raw pointer and size
    message_reader(const uint8_t* data, size_t size)
        : data_(data, size)
        , pos_(0)
    {}

    // Check if there are at least n bytes remaining
    [[nodiscard]] auto has_remaining(size_t n) const -> bool {
    ...
    [[nodiscard]] auto read_i16() -> int16_t {
    ...
    [[nodiscard]] auto read_i32() -> int32_t {
    ...
    [[nodiscard]] auto read_u64() -> uint64_t {
        ...
        uint64_t value = static_cast<uint64_t>(data_[pos_])
    ...
    [[nodiscard]] auto read_i64() -> int64_t {
    ...
    [[nodiscard]] auto remaining_data() const -> std::span<const uint8_t> {
    ...
    [[nodiscard]] auto peek_u16() const -> std::optional<uint16_t> {
        ...
        return static_cast<uint16_t>(data_[pos_])
             | (static_cast<uint16_t>(data_[pos_ + 1]) << 8);
    }
