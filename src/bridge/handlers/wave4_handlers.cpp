...
auto handle_attack(const handler_context& ctx) -> handle_result {
    ...
    LOG_DEBUG(proto_bridge, "Attack: player={} at ({},{}) dir={} target={}",
              ctx.player.value, x, y, static_cast<int>(dir), target.value);
...
auto handle_attack_target(const handler_context& ctx, entity_id target_id) -> handle_result {
    ...
    send_damage_event(ctx, target_id, result.hit.final_damage,
                      result.hit.is_critical(), result.target_killed);
...
auto handle_magic(const handler_context& ctx) -> handle_result {
    ...
    LOG_DEBUG(proto_bridge, "Magic: player={} spell={} target=({},{}) entity={}",
              ctx.player.value, spell_id_val, target_x, target_y, target.value);
...
auto handle_cast_spell(const handler_context& ctx, spell_id spell, entity_id target) -> handle_result {
    ...
    // Build cast target
    magic::cast_target cast_target{
        .target = entity::entity{target.value, 0},
        .target_pos = world::position{0, 0}
    };
...
auto handle_use_skill(const handler_context& ctx) -> handle_result {
    ...
    LOG_DEBUG(proto_bridge, "Skill: player={} skill={} target={}",
              ctx.player.value, skill_id_val, target.value);
...
auto handle_skill_use(const handler_context& ctx, skill_id skill, entity_id target) -> handle_result {
    ...
    switch (result) {
        case skill::skill_use_result::success:
            LOG_DEBUG(proto_bridge, "Skill use success: player={} skill={}",
                      ctx.player.value, skill.value);
            send_skill_result(ctx, skill, true);
            break;

        case skill::skill_use_result::cooldown:
            LOG_DEBUG(proto_bridge, "Skill on cooldown: player={} skill={}",
                      ctx.player.value, skill.value);
            send_skill_result(ctx, skill, false);
            break;

        case skill::skill_use_result::insufficient_skill:
            LOG_DEBUG(proto_bridge, "Insufficient skill level: player={} skill={}",
                      ctx.player.value, skill.value);
            send_skill_result(ctx, skill, false);
            break;
