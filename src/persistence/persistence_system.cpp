...
void persistence_system::update(float delta_time) {
    if (!config_.auto_save_enabled) return;
...
auto persistence_system::player_exists(player_id id) -> bool {
    if (player_cache_.contains(id)) return true;
...
auto persistence_system::guild_exists(guild_id id) -> bool {
    if (guild_cache_.contains(id)) return true;
...
auto persistence_system::create_backup(const std::string& name) -> persist_result {
    ...
    if (std::filesystem::exists(src_players)) {
        std::filesystem::copy(src_players, dst_players,
    ...
    if (std::filesystem::exists(src_guilds)) {
        std::filesystem::copy(src_guilds, dst_guilds,
            std::filesystem::copy_options::recursive | std::filesystem::copy_options::overwrite_existing, ec);
        if (ec) {
...
auto persistence_system::deserialize_player(binary_reader& reader) -> result<player_save_data, persist_result> {
    ...
    if (!id.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.id = player_id{id.value()};

    auto name = reader.read_string();
    if (!name.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.name = name.value();

    auto account = reader.read_string();
    if (!account.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.account_name = account.value();

    // Character info
    auto nation = reader.read_uint8();
    if (!nation.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.nation = nation.value();

    auto gender = reader.read_uint8();
    if (!gender.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.gender = gender.value();

    auto skin = reader.read_uint8();
    if (!skin.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.skin_color = skin.value();

    auto hair_style = reader.read_uint8();
    if (!hair_style.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.hair_style = hair_style.value();

    auto hair_color = reader.read_uint8();
    if (!hair_color.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.hair_color = hair_color.value();

    auto underwear = reader.read_uint8();
    if (!underwear.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.underwear = underwear.value();

    // Location
    auto map = reader.read_uint8();
    if (!map.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.current_map = map_id{map.value()};

    auto x = reader.read_int16();
    if (!x.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.x = x.value();

    auto y = reader.read_int16();
    if (!y.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.y = y.value();

    // Stats
    auto level = reader.read_int16();
    if (!level.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.level = level.value();

    auto exp = reader.read_int64();
    if (!exp.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.experience = exp.value();

    auto hp = reader.read_int32();
    if (!hp.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.hp = hp.value();

    auto max_hp = reader.read_int32();
    if (!max_hp.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.max_hp = max_hp.value();

    auto mp = reader.read_int32();
    if (!mp.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.mp = mp.value();

    auto max_mp = reader.read_int32();
    if (!max_mp.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.max_mp = max_mp.value();

    auto sp = reader.read_int32();
    if (!sp.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.sp = sp.value();

    auto max_sp = reader.read_int32();
    if (!max_sp.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.max_sp = max_sp.value();

    // Attributes
    auto str = reader.read_int16();
    if (!str.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.strength = str.value();

    auto vit = reader.read_int16();
    if (!vit.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.vitality = vit.value();

    auto dex = reader.read_int16();
    if (!dex.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.dexterity = dex.value();

    auto intel = reader.read_int16();
    if (!intel.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.intelligence = intel.value();

    auto mag = reader.read_int16();
    if (!mag.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.magic = mag.value();

    auto cha = reader.read_int16();
    if (!cha.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.charisma = cha.value();

    auto luck = reader.read_int16();
    if (!luck.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.luck = luck.value();

    // Combat
    auto ek = reader.read_int32();
    if (!ek.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.ek_count = ek.value();

    auto pk = reader.read_int32();
    if (!pk.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.pk_count = pk.value();

    auto contrib = reader.read_int32();
    if (!contrib.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.contribution = contrib.value();

    // Currency
    auto gold = reader.read_int64();
    if (!gold.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.gold = gold.value();

    auto bank_gold = reader.read_int64();
    if (!bank_gold.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.bank_gold = bank_gold.value();

    // Guild
    auto guild = reader.read_uint16();
    if (!guild.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.guild = guild_id{guild.value()};

    auto guild_rank = reader.read_uint8();
    if (!guild_rank.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.guild_rank = guild_rank.value();

    // Timestamps
    auto created = reader.read_int64();
    if (!created.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.created_timestamp = created.value();

    auto last_login = reader.read_int64();
    if (!last_login.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.last_login_timestamp = last_login.value();

    auto play_time = reader.read_int32();
    if (!play_time.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.play_time_seconds = play_time.value();

    // Serialized blobs
    auto inv_size = reader.read_uint32();
    if (!inv_size.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    auto inv_data = reader.read_bytes(inv_size.value());
    if (!inv_data.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.inventory_data = inv_data.value();

    auto skills_size = reader.read_uint32();
    if (!skills_size.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    auto skills_data = reader.read_bytes(skills_size.value());
    if (!skills_data.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.skills_data = skills_data.value();

    auto quests_size = reader.read_uint32();
    if (!quests_size.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    auto quests_data = reader.read_bytes(quests_size.value());
    if (!quests_data.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    data.quests_data = quests_data.value();

    auto equip_size = reader.read_uint32();
    if (!equip_size.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
    auto equip_data = reader.read_bytes(equip_size.value());
    if (!equip_data.is_ok()) return result<player_save_data, persist_result>::err(persist_result::parse_error);
...
auto persistence_system::deserialize_guild(binary_reader& reader) -> result<guild_save_data, persist_result> {
    ...
    if (!id.is_ok()) return result<guild_save_data, persist_result>::err(persist_result::parse_error);
    data.id = guild_id{id.value()};

    auto name = reader.read_string();
    if (!name.is_ok()) return result<guild_save_data, persist_result>::err(persist_result::parse_error);
    data.name = name.value();

    auto tag = reader.read_string();
    if (!tag.is_ok()) return result<guild_save_data, persist_result>::err(persist_result::parse_error);
    data.tag = tag.value();

    auto motd = reader.read_string();
    if (!motd.is_ok()) return result<guild_save_data, persist_result>::err(persist_result::parse_error);
    data.motd = motd.value();

    auto master = reader.read_uint32();
    if (!master.is_ok()) return result<guild_save_data, persist_result>::err(persist_result::parse_error);
    data.master = player_id{master.value()};

    auto faction = reader.read_uint8();
    if (!faction.is_ok()) return result<guild_save_data, persist_result>::err(persist_result::parse_error);
    data.faction = faction.value();

    // Stats
    auto gold = reader.read_int64();
    if (!gold.is_ok()) return result<guild_save_data, persist_result>::err(persist_result::parse_error);
    data.gold_bank = gold.value();

    auto level = reader.read_int32();
    if (!level.is_ok()) return result<guild_save_data, persist_result>::err(persist_result::parse_error);
    data.level = level.value();

    auto exp = reader.read_int64();
    if (!exp.is_ok()) return result<guild_save_data, persist_result>::err(persist_result::parse_error);
    data.experience = exp.value();

    auto kills = reader.read_int64();
    if (!kills.is_ok()) return result<guild_save_data, persist_result>::err(persist_result::parse_error);
    data.total_kills = kills.value();

    auto deaths = reader.read_int64();
    if (!deaths.is_ok()) return result<guild_save_data, persist_result>::err(persist_result::parse_error);
    data.total_deaths = deaths.value();

    auto created = reader.read_int64();
    if (!created.is_ok()) return result<guild_save_data, persist_result>::err(persist_result::parse_error);
    data.created_timestamp = created.value();

    // Serialized blobs
    auto members_size = reader.read_uint32();
    if (!members_size.is_ok()) return result<guild_save_data, persist_result>::err(persist_result::parse_error);
    auto members_data = reader.read_bytes(members_size.value());
    if (!members_data.is_ok()) return result<guild_save_data, persist_result>::err(persist_result::parse_error);
    data.members_data = members_data.value();

    auto ranks_size = reader.read_uint32();
    if (!ranks_size.is_ok()) return result<guild_save_data, persist_result>::err(persist_result::parse_error);
    auto ranks_data = reader.read_bytes(ranks_size.value());
    if (!ranks_data.is_ok()) return result<guild_save_data, persist_result>::err(persist_result::parse_error);
...
auto persistence_system::get_player_path(player_id id) const -> std::filesystem::path {
    return config_.save_directory / config_.player_directory /
           (std::to_string(id.value) + ".dat");
}

auto persistence_system::get_guild_path(guild_id id) const -> std::filesystem::path {
    return config_.save_directory / config_.guild_directory /
           (std::to_string(id.value) + ".dat");
}

auto persistence_system::write_file(const std::filesystem::path& path, const std::vector<uint8_t>& data) -> persist_result {
...
void persistence_system::do_auto_save() {
    if (save_providers_.empty()) return;
...
void persistence_system::cleanup_old_backups() {
    if (config_.max_backups <= 0) return;

    auto backups = list_backups();
    if (backups.size() <= static_cast<size_t>(config_.max_backups)) return;
