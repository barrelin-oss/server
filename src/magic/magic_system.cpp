...
auto magic_system::begin_cast(hb::entity::entity caster,
                              spell_id spell_id,
...
auto magic_system::instant_cast(hb::entity::entity caster,
                                spell_id spell_id,
...
auto magic_system::can_cast(hb::entity::entity caster,
                            spell_id spell_id,
                            const cast_target& target) const -> cast_result
{
    if (!caster.is_valid())
    {
    ...
    if (player_sys)
    {
        if (auto* p = player_sys->get_player(player_id{caster.id}))
        {
            ...
            if (p->computed.intelligence < spell->int_requirement || p->computed.magic < spell->mag_requirement)
            {
                return cast_result::level_too_low;
            }
        }
    ...
    if (spell->is_offensive() && !spell->is_aoe() && target.has_entity() && target.target.id == caster.id)
    {
...
void magic_system::forget_spell(hb::entity::entity caster, spell_id spell)
{
    ...
    std::erase_if(spells, [spell](const spell_knowledge& k) { return k.spell == spell; });
}
...
auto magic_system::apply_spell_effect(hb::entity::entity caster,
                                      ...
                                      const cast_target& target) -> spell_effect_result
{
        ...
        for (auto& t : targets)
        {
            if (player_sys)
            {
                ...
                if (target_player && is_resurrection && target_player->is_dead())
                {
                    ...
                    int32_t restore_hp =
                        result.heal_applied > 0 ? result.heal_applied : target_player->computed.max_hp / 2;
    ...
    LOG_DEBUG(general,
              "Entity {} cast spell '{}' (damage: {}, heal: {}, targets: {})",
              caster.id,
              spell.name,
              result.damage_dealt,
              result.heal_applied,
              result.affected_targets.size());

    return result;
}

void magic_system::notify_spell_cast(hb::entity::entity caster,
                                     const spell_template& spell,
                                     const spell_effect_result& result)
{
...
auto magic_system::find_aoe_targets(hb::entity::entity caster,
                                    const spell_template& spell,
                                    const cast_target& target) const -> std::vector<hb::entity::entity>
{
    ...
    if (spell.target_type == spell_target::aoe_ally || spell.target_type == spell_target::aoe_all)
    {
        if (player_sys)
        {
            player_sys->for_each_player(
...
auto magic_system::element_to_damage_type(spell_element element) const -> combat::damage_type
{
    switch (element)
    {
    case spell_element::fire:
        return combat::damage_type::fire;
    case spell_element::ice:
        return combat::damage_type::ice;
    case spell_element::lightning:
        return combat::damage_type::lightning;
    case spell_element::holy:
        return combat::damage_type::holy;
    case spell_element::dark:
        return combat::damage_type::dark;
    default:
        return combat::damage_type::magic;
    }
}
