...
void item_audit_system::log_item(int32_t character_id, std::string_view item_name,
                                  int32_t item_instance_id, item_log_type action,
                                  int32_t quantity, int32_t other_char_id,
...
void item_audit_system::log_gold(int32_t character_id, item_log_type action,
                                  int32_t gold_amount, int32_t other_char_id,
                                  std::string_view map_name,
                                  int16_t pos_x, int16_t pos_y,
                                  const nlohmann::json& details)
{
...
{
    if (!db_ || entries.empty()) return;

    auto result = db_->execute_transaction([&](pqxx::work& txn) -> hb::result<void, std::string> {
...
auto item_audit_system::query_logs(const item_log_query& query) -> std::vector<item_log_result>
{
    ...
    sql << "SELECT il.id, il.character_id, COALESCE(c1.name, '') AS character_name, "
        << "il.item_name, il.item_id, il.action_type, il.quantity, "
        << "COALESCE(il.gold_amount, 0) AS gold_amount, "
        << "il.other_char_id, COALESCE(c2.name, '') AS other_char_name, "
        << "COALESCE(il.map_name, '') AS map_name, "
        << "COALESCE(il.pos_x, 0) AS pos_x, COALESCE(il.pos_y, 0) AS pos_y, "
        << "il.timestamp, COALESCE(il.details, '{}'::jsonb) AS details "
        << "FROM item_log il "
        << "LEFT JOIN characters c1 ON il.character_id = c1.id "
        << "LEFT JOIN characters c2 ON il.other_char_id = c2.id "
        << "WHERE 1=1 ";
    ...
    auto txn_result = db_->execute_transaction([&](pqxx::work& txn) -> hb::result<void, std::string> {
