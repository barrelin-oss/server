...
public:
    ...
    [[nodiscard]] static auto err(E error) -> result {
    ...
    template<typename F>
    [[nodiscard]] auto value_or_else(F&& f) const& -> T {
    ...
    template<typename F>
    [[nodiscard]] auto or_else(F&& f) const& -> result {
        if (is_err()) {
