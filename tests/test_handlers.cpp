...
TEST(handle_result_test, enum_values) {
    ...
    EXPECT_NE(static_cast<int>(handle_result::handled),
              static_cast<int>(handle_result::handled_async));
    EXPECT_NE(static_cast<int>(handle_result::handled),
              static_cast<int>(handle_result::not_handled));
    EXPECT_NE(static_cast<int>(handle_result::handled),
              static_cast<int>(handle_result::error));
    EXPECT_NE(static_cast<int>(handle_result::handled),
              static_cast<int>(handle_result::disconnect));
}

TEST(handle_result_test, can_use_in_switch) {
    auto test_result = [](handle_result r) -> std::string {
        switch (r) {
            case handle_result::handled:      return "handled";
            case handle_result::handled_async: return "async";
            case handle_result::not_handled:  return "not_handled";
            case handle_result::error:        return "error";
            case handle_result::disconnect:   return "disconnect";
...
TEST(message_stats_test, time_tracking) {
    message_stats stats{
        .total_time = std::chrono::nanoseconds{5000000},
        .max_time = std::chrono::nanoseconds{100000}
    };
...
TEST(router_stats_test, time_tracking) {
    router_stats stats{
        .total_messages = 1000,
        .total_processing_time = std::chrono::nanoseconds{10000000}
    };
