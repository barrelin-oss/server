...
void network_subsystem::accept_connections()
{
    ...
    while (true)
    {
        ...
        {
            LOG_WARN(network, "Connection limit reached, rejecting {}:{}", address.ip, address.port);
            socket.close();
            continue;
        }
        ...
        LOG_INFO(network, "Client connected: {} ({}:{})", id.value, address.ip, address.port);

        // Publish event
        event_bus().publish(
            client_connected_event{.id = id, .address = address, .timestamp = std::chrono::system_clock::now()});
    }
}
...
void network_subsystem::process_connections()
{
    ...
    connections_.for_each(
        [this, &to_remove](connection& conn)
        {
            if (!conn.is_connected())
            {
            ...
            while (conn.has_pending_message())
            {
                ...
                else
                {
                    ...
                    event_bus().publish(data_received_event{.id = conn.id(), .data = std::move(message)});
                }
            }
        });
...
void network_subsystem::check_timeouts()
{
    ...
    connections_.for_each(
        [this, &timed_out](connection& conn)
        {
...
void network_subsystem::handle_disconnect(connection_id id, std::string_view reason)
{
    ...
    LOG_INFO(network, "Client disconnected: {} ({}:{}) - {}", id.value, address.ip, address.port, reason);
