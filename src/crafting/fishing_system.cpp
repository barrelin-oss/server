...
auto fishing_system::check_fish_nearby(entity_id player_eid,
                                       ...
                                       int16_t player_y) -> std::optional<uint32_t>
{
    ...
    for (uint32_t i = 1; i < max_fish_nodes; ++i)
    {
        ...
        if (fish.engaging_count >= max_engaging_fish)
            continue;
...
auto fishing_system::attempt_catch(entity_id player_eid) -> fish_catch_result
{
    ...
    if (roll <= plr->fishing.catch_chance)
    {
        ...
        if (fish.config)
        {
            ...
            {
                auto item_result =
                    items_->create_from_template(item_id{static_cast<uint32_t>(fish.config->template_id)}, 1);
...
auto fishing_system::calculate_chance_change(int16_t skill, int16_t difficulty) -> std::pair<int32_t, int32_t>
{
...
void fishing_system::generate_fish()
{
    ...
    world_->for_each_map(
        [this](map_id /*id*/, world::map& m)
        {
            ...
            auto lifespan_minutes = std::clamp(fish_config->lifespan_min + lifespan_var(rng), 10, 30);

            fish_node node{.index = slot_idx,
...
void fishing_system::update_fishing_chances()
{
    ...
    players_->for_each_player(
        [this](player_id /*pid*/, player::player& plr)
        {
            thread_local std::mt19937 rng{std::random_device{}()};
...
void fishing_system::delete_fish_node(uint32_t index, catch_result reason)
{
    ...
    {
        players_->for_each_player(
            [this, index, reason](player_id /*pid*/, player::player& plr)
            {
