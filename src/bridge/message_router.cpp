...
message_router::message_router() : impl_(std::make_unique<impl>()) {}
...
void message_router::register_handler(protocol::message_id msg_id, std::string_view subsystem, message_handler handler)
{
    register_handler(handler_info{.name = std::string(protocol::message_id_string(msg_id)),
...
void message_router::unregister_handler(protocol::message_id msg_id)
{
    ...
    {
        LOG_DEBUG(proto_bridge, "Unregistered handler for {}", protocol::message_id_string(msg_id));
...
void message_router::set_handler_enabled(protocol::message_id msg_id, bool enabled)
{
    ...
    if (it != impl_->handlers.end())
    {
        ...
        LOG_DEBUG(
            proto_bridge, "Handler for {} {}", protocol::message_id_string(msg_id), enabled ? "enabled" : "disabled");
    }
}
...
auto message_router::get_handler_info(protocol::message_id msg_id) const -> const handler_info*
{
    std::shared_lock lock(impl_->handler_mutex);
    auto it = impl_->handlers.find(msg_id);
    if (it == impl_->handlers.end())
...
auto message_router::route(const handler_context& ctx) -> bool
{
    ...
    if (handler_found && handler_enabled && handler_copy)
    {
        if (impl_->verbose_logging)
        {
            LOG_TRACE(proto_bridge,
                      "Routing {} to modern handler (conn={})",
        ...
        {
            LOG_ERROR(proto_bridge, "Handler exception for {}: {}", protocol::message_id_string(ctx.msg_id), e.what());
    ...
    {
        LOG_TRACE(proto_bridge,
                  "Falling back to legacy for {} (conn={})",
                  protocol::message_id_string(ctx.msg_id),
                  ctx.connection.value);
    }
    ...
    if (impl_->fallback)
    {
        ...
        {
            LOG_ERROR(proto_bridge,
                      "Legacy fallback exception for {}: {}",
                      protocol::message_id_string(ctx.msg_id),
                      e.what());
        }
    }
...
auto message_router::route(connection_id conn, std::span<const uint8_t> data) -> bool
{
    ...
    if (data.size() < 4)
    {
        LOG_WARN(proto_bridge, "Message too short ({} bytes) from conn={}", data.size(), conn.value);
        return false;
    }

    // Extract message ID (first 4 bytes, little-endian)
    uint32_t msg_id_raw = static_cast<uint32_t>(data[0]) | (static_cast<uint32_t>(data[1]) << 8) |
    ...
    player_id player{};
    {
        ...
        if (auto it = impl_->connection_sessions.find(conn); it != impl_->connection_sessions.end())
        {
            session = it->second;
        }
        if (auto it = impl_->connection_players.find(conn); it != impl_->connection_players.end())
        {
            player = it->second;
        }
    }
    ...
    // Build context
    handler_context ctx{.connection = conn,
                        .session = session,
                        .player = player,
                        .msg_id = msg_id,
                        .raw_data = data,
...
{
    std::call_once(g_router_init, []() { g_router = std::make_unique<message_router>(); });
    return *g_router;
}
