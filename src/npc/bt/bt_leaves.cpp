...
{
    if (!npc_ref.ai_state.target.is_valid()) return node_status::failure;
    if (!ctx.npc_sys) return node_status::failure;
...
auto attack_target::tick(npc& npc_ref, bt_context& ctx) -> node_status
{
    ...
    auto time_since_attack = std::chrono::duration_cast<std::chrono::milliseconds>(
        now - npc_ref.ai_state.last_attack_time).count();

    int attack_cooldown = npc_ref.attack_speed > 0
        ? 1000 * 100 / npc_ref.attack_speed
        : 1000;
...
{
    if (!ctx.npc_sys) return node_status::failure;
    if (script_name.empty()) return node_status::failure;
