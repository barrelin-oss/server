...
template<typename T> auto yaml_get(const YAML::Node& node, const std::string& key, T default_value) -> T
{
    if (node[key])
    {
        try
    ...
    -> result<std::unordered_map<std::string, std::string>, std::string>
{
    ...
    if (!file.is_open())
    {
        return result<std::unordered_map<std::string, std::string>, std::string>::err("Failed to open file: " +
                                                                                      path.string());
    }
...
auto config_system::load_server_config(const std::filesystem::path& path) -> result<void, std::string>
{
...
auto config_system::load_yaml_config(const std::filesystem::path& path) -> result<void, std::string>
{
    try
    {
...
auto config_system::load_ini_config(const std::filesystem::path& path) -> result<void, std::string>
{
...
auto server_config::load_from_file(const std::filesystem::path& path) -> result<server_config, std::string>
{
    auto parse_result = parse_ini_file(path);
    if (parse_result.is_err())
    {
...
auto server_config::load_from_json(const std::filesystem::path& path) -> result<server_config, std::string>
{
    ...
    if (!file.is_open())
    {
        return result<server_config, std::string>::err("Failed to open JSON config: " + path.string());
    }
    ...
    try
    {
        ...
        if (j.contains("server_name"))
            config.server_name = j["server_name"];
        if (j.contains("game_server_port"))
            config.game_server_port = j["game_server_port"];
        if (j.contains("log_server_addr"))
            config.log_server_addr = j["log_server_addr"];
        if (j.contains("log_server_port"))
            config.log_server_port = j["log_server_port"];
        if (j.contains("gate_server_addr"))
    ...
    {
        return result<server_config, std::string>::err("JSON parse error: " + std::string(e.what()));
    }
}

auto server_config::save_to_json(const std::filesystem::path& path) const -> result<void, std::string>
{
    nlohmann::json j;
    ...
    if (!file.is_open())
    {
        return result<void, std::string>::err("Failed to open file for writing: " + path.string());
    }
...
auto server_config::to_json() const -> nlohmann::json
{
    ...
    j["websocket"] = {{"bind_address", websocket.bind_address},
                      {"port", websocket.port},
    ...
    j["auth"] = {{"max_characters_per_account", auth.max_characters_per_account},
    ...
    j["forum_auth"] = {{"enabled", forum_auth.enabled},
                       {"login_url", forum_auth.login_url},
                       {"validate_url", forum_auth.validate_url},
                       {"api_key", forum_auth.api_key}};

    j["auto_save"] = {{"enabled", auto_save.enabled}, {"interval_seconds", auto_save.interval_seconds}};

    j["logging"] = {{"console_level", logging.console_level},
...
auto server_config::apply_dot_values(const nlohmann::json& values) -> result<std::vector<std::string>, std::string>
{
    ...
    for (auto& [key, value] : values.items())
    {
        ...
        if (section == "database")
        {
            if (field == "host" && value.is_string())
            {
                database.host = value;
                matched = true;
            }
            else if (field == "port" && value.is_number())
            {
                database.port = value;
                matched = true;
            }
            else if (field == "database" && value.is_string())
            {
                database.database = value;
                matched = true;
            }
            else if (field == "username" && value.is_string())
            {
                database.username = value;
                matched = true;
            }
            else if (field == "password" && value.is_string())
            {
                database.password = value;
                matched = true;
            }
            else if (field == "pool_size" && value.is_number())
            {
                database.pool_size = value;
                matched = true;
            }
        }
        else if (section == "websocket")
        {
            if (field == "bind_address" && value.is_string())
            {
                websocket.bind_address = value;
                matched = true;
            }
            else if (field == "port" && value.is_number())
            {
                websocket.port = value;
                matched = true;
            }
            else if (field == "max_connections" && value.is_number())
            {
                websocket.max_connections = value;
                matched = true;
            }
            else if (field == "enable_ping" && value.is_boolean())
            {
                websocket.enable_ping = value;
                matched = true;
            }
            else if (field == "ping_interval_seconds" && value.is_number())
            {
                websocket.ping_interval_seconds = value;
                matched = true;
            }
        }
        else if (section == "auth")
        {
            if (field == "allow_registration" && value.is_boolean())
            {
                auth.allow_registration = value;
                matched = true;
            }
            else if (field == "max_login_attempts" && value.is_number())
            {
                auth.max_login_attempts = value;
                matched = true;
            }
        }
        else if (section == "auto_save")
        {
            ...
            {
                auto_save.enabled = value;
                matched = true;
            }
            else if (field == "interval_seconds" && value.is_number())
            {
                auto_save.interval_seconds = value;
                matched = true;
            }
        }
        else if (section == "logging")
        {
            if (field == "console_level" && value.is_string())
            {
                logging.console_level = value;
                matched = true;
            }
            else if (field == "file_level" && value.is_string())
            {
                logging.file_level = value;
                matched = true;
            }
        }
        else if (section == "forum_auth")
        {
            if (field == "enabled" && value.is_boolean())
            {
                forum_auth.enabled = value;
                matched = true;
            }
            else if (field == "api_key" && value.is_string())
            {
                forum_auth.api_key = value;
                matched = true;
            }
        }
...
auto admin_config::load_from_file(const std::filesystem::path& path) -> result<admin_config, std::string>
{
    auto parse_result = parse_ini_file(path);
    if (parse_result.is_err())
    {
