...
auto serialize_equipment(const player::equipment_state& equipment) -> std::string
{
    ...
    for (size_t i = 0; i < player::equip_slot_count; ++i)
    {
        ...
        if (!item.is_empty())
        {
            j.push_back({{"slot", static_cast<int>(i)},
...
auto serialize_equipment_with_attributes(const player::equipment_state& equipment,
                                         const std::vector<equipment_slot_attribute>& attributes) -> std::string
{
    ...
    for (size_t i = 0; i < player::equip_slot_count; ++i)
    {
        ...
        if (!item.is_empty())
        {
            nlohmann::json slot_obj = {{"slot", static_cast<int>(i)},
                                       {"item_id", item.id.value},
                                       {"durability", item.durability},
                                       {"max_durability", item.max_durability}};
...
auto serialize_inventory(const inventory::inventory& inv) -> std::string
{
    ...
    for (int16_t i = 0; i < inv.capacity(); ++i)
    {
        ...
        {
            j.push_back({{"slot", i}, {"item_id", slot->item.value}, {"count", slot->count}});
        }
    }
...
auto get_inventory_data(const inventory::inventory& inv) -> std::vector<inventory_slot_data>
{
    ...
    for (int16_t i = 0; i < inv.capacity(); ++i)
    {
        ...
        if (slot && !slot->is_empty())
        {
            data.push_back({.slot = i, .item_id = slot->item.value, .count = slot->count});
        }
    }
...
auto serialize_inventory_with_attributes(const inventory::inventory& inv,
                                         const std::vector<inventory_slot_attribute>& attributes) -> std::string
{
    ...
    for (int16_t i = 0; i < inv.capacity(); ++i)
    {
        ...
        if (slot && !slot->is_empty())
        {
            nlohmann::json slot_obj = {{"slot", i}, {"item_id", slot->item.value}, {"count", slot->count}};
