...
class binary_writer {
public:
    explicit binary_writer(size_t initial_capacity = 1024) {
    ...
    // Move buffer out
    [[nodiscard]] auto release() -> std::vector<uint8_t> {
...
public:
    ...
    auto read_uint64() -> result<uint64_t, serialize_error> {
        ...
        if (!low.is_ok()) return result<uint64_t, serialize_error>::err(low.error());

        auto high = read_uint32();
        if (!high.is_ok()) return result<uint64_t, serialize_error>::err(high.error());

        return result<uint64_t, serialize_error>::ok(
            static_cast<uint64_t>(low.value()) | (static_cast<uint64_t>(high.value()) << 32));
    }

    auto read_int8() -> result<int8_t, serialize_error> {
        auto res = read_uint8();
        if (!res.is_ok()) return result<int8_t, serialize_error>::err(res.error());
    ...
    auto read_int16() -> result<int16_t, serialize_error> {
        ...
        if (!res.is_ok()) return result<int16_t, serialize_error>::err(res.error());
    ...
    auto read_int32() -> result<int32_t, serialize_error> {
        ...
        if (!res.is_ok()) return result<int32_t, serialize_error>::err(res.error());
    ...
    auto read_int64() -> result<int64_t, serialize_error> {
        ...
        if (!res.is_ok()) return result<int64_t, serialize_error>::err(res.error());
    ...
    auto read_float() -> result<float, serialize_error> {
        ...
        if (!bits.is_ok()) return result<float, serialize_error>::err(bits.error());
        float value;
    ...
    auto read_double() -> result<double, serialize_error> {
        ...
        if (!bits.is_ok()) return result<double, serialize_error>::err(bits.error());
        double value;
    ...
    auto read_bool() -> result<bool, serialize_error> {
        auto res = read_uint8();
        if (!res.is_ok()) return result<bool, serialize_error>::err(res.error());
    ...
    auto read_string() -> result<std::string, serialize_error> {
        ...
        if (!length_res.is_ok()) return result<std::string, serialize_error>::err(length_res.error());
    ...
    void seek(size_t pos) {
        position_ = std::min(pos, data_.size());
    }
