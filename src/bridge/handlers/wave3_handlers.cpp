...
void send_notify(const handler_context& ctx,
                 protocol::notify_type type,
                 std::function<void(protocol::message_writer&)> write_data)
{
    send_response(ctx,
                  protocol::message_id::notify,
                  [type, &write_data](protocol::message_writer& writer)
                  {
...
void send_motion_response(const handler_context& ctx, motion_type type, int16_t x, int16_t y, int8_t dir)
{
    send_response(ctx,
                  protocol::message_id::response_motion,
...
auto handle_motion_command(const handler_context& ctx) -> handle_result
{
    ...
    LOG_DEBUG(proto_bridge,
              "Motion command: type={} pos=({},{}) dir={} (player={})",
...
auto handle_motion_move(const handler_context& ctx, int16_t x, int16_t y, int8_t dir, uint8_t run_mode) -> handle_result
{
    LOG_TRACE(proto_bridge,
              "Move: player={} to ({},{}) dir={} run={}",
              ctx.player.value,
              x,
              y,
              static_cast<int>(dir),
    ...
    if (x < 0 || y < 0)
    {
        LOG_WARN(proto_bridge, "Invalid move position ({},{}) for player {}", x, y, ctx.player.value);
    ...
    case move_result::teleport:
    {
        ...
        LOG_INFO(proto_bridge,
                 "Player {} teleporting to {} at ({},{})",
                 ctx.player.value,
                 move_info.teleport_dest_map,
                 move_info.teleport_dest_pos.x,
                 move_info.teleport_dest_pos.y);
    ...
    case move_result::blocked_terrain:
        LOG_DEBUG(proto_bridge, "Move blocked (terrain) for player {} at ({},{})", ctx.player.value, x, y);
        // Send player back to original position
        send_motion_response(ctx, motion_type::stop, player->pos.x, player->pos.y, static_cast<int8_t>(player->facing));
        return handle_result::handled;

    case move_result::blocked_occupied:
        LOG_DEBUG(proto_bridge, "Move blocked (occupied) for player {} at ({},{})", ctx.player.value, x, y);
        send_motion_response(ctx, motion_type::stop, player->pos.x, player->pos.y, static_cast<int8_t>(player->facing));
        return handle_result::handled;

    case move_result::blocked_out_of_bounds:
        LOG_DEBUG(proto_bridge, "Move blocked (out of bounds) for player {} at ({},{})", ctx.player.value, x, y);
        send_motion_response(ctx, motion_type::stop, player->pos.x, player->pos.y, static_cast<int8_t>(player->facing));
        return handle_result::handled;

    case move_result::blocked_status:
        LOG_DEBUG(proto_bridge, "Move blocked (status effect) for player {}", ctx.player.value);
...
{
    LOG_TRACE(proto_bridge, "Stop: player={} at ({},{}) dir={}", ctx.player.value, x, y, static_cast<int>(dir));
...
auto handle_drop_item(const handler_context& ctx) -> handle_result
{
    ...
    LOG_DEBUG(proto_bridge, "Drop item: player={} slot={} count={}", ctx.player.value, slot, count);
...
auto handle_pickup_item(const handler_context& ctx) -> handle_result
{
    ...
    if (result != inventory::inventory_result::success)
    {
        ...
        LOG_WARN(proto_bridge,
                 "Failed to add item {} to player {} inventory: result={}",
    ...
    send_notify(ctx,
                protocol::notify_type::item_obtained,
                [&](protocol::message_writer& writer)
                {
    ...
    LOG_INFO(proto_bridge,
             "Player {} picked up item {} at ({}, {}) on map {}",
             ctx.player.value,
             picked_item_id.value,
             player.pos.x,
             player.pos.y,
...
void broadcast_item_removal(const handler_context& ctx, const player::player& picker, item_id removed_item)
{
    ...
    LOG_DEBUG(proto_bridge, "Broadcasted item {} removal to {} nearby players", removed_item.value, nearby.size() - 1);
}
