...
public:
    ...
    [[nodiscard]] static auto now() -> time_point { return std::chrono::steady_clock::now(); }
    ...
    [[nodiscard]] static auto elapsed_since_ms(time_point start) -> int64_t { return elapsed_ms(start, now()); }
    ...
    static void sleep_for(std::chrono::milliseconds duration) { std::this_thread::sleep_for(duration); }

    // Sleep until a time point
    static void sleep_until(time_point tp) { std::this_thread::sleep_until(tp); }
};
