...
protected:
    ...
    auto make_test_message(message_id id, std::function<void(message_writer&)> write_data = nullptr)
        -> std::vector<uint8_t> {
...
TEST_F(message_router_test, register_handler) {
    router_->register_handler(
        message_id::request_login,
        "test_subsystem",
        [](const handler_context&) { return handle_result::handled; }
    );
...
TEST_F(message_router_test, register_handler_with_info) {
    router_->register_handler(handler_info{
        .name = "login_handler",
        .subsystem = "auth",
...
TEST_F(message_router_test, route_to_legacy_fallback) {
    ...
    router_->set_legacy_fallback([&](connection_id conn, std::span<const uint8_t>) {
        legacy_called = true;
        received_conn = conn;
    });
...
TEST_F(message_router_test, stats_track_errors) {
    router_->register_handler(
        message_id::request_login,
        "test",
        [](const handler_context&) { return handle_result::error; }
    );
...
TEST_F(message_router_test, message_data_passed_to_handler) {
    ...
    auto data = make_test_message(message_id::request_login, [](message_writer& w) {
        w.write_u16(0x1234);
...
TEST_F(message_router_test, get_all_handlers) {
    router_->register_handler(message_id::request_login, "auth",
        [](const handler_context&) { return handle_result::handled; });
    router_->register_handler(message_id::request_enter_game, "game",
        [](const handler_context&) { return handle_result::handled; });
    router_->register_handler(message_id::notify, "notify",
        [](const handler_context&) { return handle_result::handled; });
...
TEST(migration_tracking_test, get_migrated_messages) {
    ...
    bool has_login = std::find(migrated.begin(), migrated.end(),
                               message_id::request_login) != migrated.end();
    bool has_notify = std::find(migrated.begin(), migrated.end(),
                                message_id::notify) != migrated.end();
...
TEST_F(wave1_handlers_test, non_help_common_falls_through) {
    ...
    bool legacy_called = false;
    router().set_legacy_fallback([&](connection_id, std::span<const uint8_t>) {
        legacy_called = true;
    });
...
protected:
    ...
    auto make_motion_message(motion_type type, int16_t x, int16_t y, int8_t dir)
        -> std::vector<uint8_t> {
        message_writer writer;
...
protected:
    ...
    auto make_party_message(uint8_t operation, uint32_t party_or_player_id = 0)
        -> std::vector<uint8_t> {
        message_writer writer;
    ...
    auto make_exchange_message(common_type subtype, uint32_t target_id = 0)
        -> std::vector<uint8_t> {
        message_writer writer;
        writer.write_u32(static_cast<uint32_t>(message_id::command_common));
