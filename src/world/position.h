...
template<typename T> [[nodiscard]] constexpr auto constexpr_abs(T x) -> T
{
    return x < 0 ? -x : x;
}
...
struct rect
{
    ...
    [[nodiscard]] constexpr auto width() const -> int16_t { return static_cast<int16_t>(max_x - min_x + 1); }

    [[nodiscard]] constexpr auto height() const -> int16_t { return static_cast<int16_t>(max_y - min_y + 1); }
...
{
    return rect{static_cast<int16_t>(center.x - radius),
...
template<> struct std::hash<hb::world::position>
{
    auto operator()(const hb::world::position& pos) const noexcept -> size_t
    {
        // Combine x and y into a single hash
        return std::hash<uint32_t>{}((static_cast<uint32_t>(static_cast<uint16_t>(pos.x)) << 16) |
