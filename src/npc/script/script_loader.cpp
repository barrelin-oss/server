...
auto parse_action_type(const std::string& str) -> action_type
{
    ...
    if (str == "face")
        return action_type::face;
    if (str == "say")
        return action_type::say;
    if (str == "broadcast")
        return action_type::broadcast;
    if (str == "emote")
        return action_type::emote;
    if (str == "spawn_npc")
        return action_type::spawn_npc;
    if (str == "despawn")
        return action_type::despawn;
    if (str == "set_state")
        return action_type::set_state;
    if (str == "set_flag")
        return action_type::set_flag;
    if (str == "apply_buff")
        return action_type::apply_buff;
    if (str == "damage_area")
        return action_type::damage_area;
    if (str == "heal_self")
        return action_type::heal_self;
    if (str == "teleport")
        return action_type::teleport;
    if (str == "set_aggro_range")
        return action_type::set_aggro_range;
    if (str == "set_invincible")
        return action_type::set_invincible;
    if (str == "play_effect")
        return action_type::play_effect;
    if (str == "call_script")
        return action_type::call_script;
    return action_type::wait;
}
...
{
    if (str == "north")
        return hb::world::direction::north;
    if (str == "north_east")
        return hb::world::direction::north_east;
    if (str == "east")
        return hb::world::direction::east;
    if (str == "south_east")
        return hb::world::direction::south_east;
    if (str == "south")
        return hb::world::direction::south;
    if (str == "south_west")
        return hb::world::direction::south_west;
    if (str == "west")
        return hb::world::direction::west;
    if (str == "north_west")
        return hb::world::direction::north_west;
    return hb::world::direction::south;
}
...
auto script_loader::load_from_file(const std::filesystem::path& path) -> result<npc_script, std::string>
{
    try
    {
        ...
        {
            return result<npc_script, std::string>::err("Missing or invalid 'actions' in script: " + path.string());
        }
    ...
    {
        return result<npc_script, std::string>::err(std::string("YAML error loading script: ") + e.what());
    }
    catch (const std::exception& e)
    {
        return result<npc_script, std::string>::err(std::string("Error loading script: ") + e.what());
    }
}

auto script_loader::load_directory(const std::filesystem::path& dir) -> result<size_t, std::string>
{
    scripts_dir_ = dir;
    ...
    for (const auto& entry : std::filesystem::directory_iterator(dir))
    {
        ...
        {
            LOG_WARN(general, "Failed to load script {}: {}", entry.path().string(), res.error());
        }
    }
