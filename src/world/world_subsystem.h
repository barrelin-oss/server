...
public:
    ...
    // Entity queries
    [[nodiscard]] auto
    get_entities_in_range(map_id map, const position& center, int radius) const -> std::vector<entity_id>;
    ...
    auto
    remove_expired_ground_items(std::chrono::seconds max_age) -> std::vector<std::tuple<map_id, position, item_id>>;
...
private:
    [[nodiscard]] auto next_map_id() -> map_id { return map_id{next_id_++}; }
    ...
    struct map_position_key
    {
        ...
        auto operator==(const map_position_key& other) const -> bool { return map == other.map && pos == other.pos; }
    };
    ...
    struct map_position_hash
    {
        ...
        {
            return std::hash<uint8_t>{}(key.map.value) ^ (std::hash<int16_t>{}(key.pos.x) << 1) ^
