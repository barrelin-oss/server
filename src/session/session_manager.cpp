...
auto session_manager::create_session(connection_id connection) -> result<session_id, std::string>
{
    ...
    LOG_INFO(general, "Session created: {} for connection {}", id.value, connection.value);

    // Publish event (outside lock would be safer but adds complexity)
    event_bus().publish(
        session_created_event{.id = id, .connection = connection, .timestamp = std::chrono::system_clock::now()});
...
void session_manager::destroy_session(session_id id, std::string_view reason)
{
    ...
    LOG_INFO(general, "Session destroyed: {} - {}", id.value, reason.empty() ? "no reason" : reason);

    event_bus().publish(session_destroyed_event{
        .id = id, .reason = std::string(reason), .timestamp = std::chrono::system_clock::now()});
}
...
void session_manager::set_session_state(session_id id, session_state state)
{
    ...
    LOG_DEBUG(general, "Session {} state changed: {} -> {}", id.value, state_string(old_state), state_string(state));

    event_bus().publish(session_state_changed_event{
        .id = id, .old_state = old_state, .new_state = state, .timestamp = std::chrono::system_clock::now()});
}
...
void session_manager::authenticate_session(session_id id, account_info account)
{
    ...
    LOG_INFO(general,
             "Session {} authenticated as {} (admin level {})",
             id.value,
             account.account_name,
             account.admin_level);

    event_bus().publish(
        session_authenticated_event{.id = id, .account = account, .timestamp = std::chrono::system_clock::now()});
}
...
void session_manager::enter_game(session_id id, player_id player)
{
    ...
    LOG_INFO(general, "Session {} entered game with player {}", id.value, player.value);
}
...
void session_manager::check_timeouts()
{
...

    {
        ...
        for (const auto& [id, sess] : sessions_)
        {
            auto idle_time = std::chrono::duration_cast<std::chrono::milliseconds>(now - sess->last_activity());
