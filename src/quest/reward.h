...
struct quest_rewards
{
    ...
    {
        return experience > 0 || gold > 0 || !items.empty() || !skill_experience.empty() || !reputation.empty() ||
               !title.empty() || unlock_flags != 0;
    }

    [[nodiscard]] auto item_count() const -> size_t { return items.size(); }
};
