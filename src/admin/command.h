...
inline auto to_string(admin_level level) -> std::string_view
{
    switch (level)
    {
    case admin_level::player:
        return "Player";
    case admin_level::helper:
        return "Helper";
    case admin_level::moderator:
        return "Moderator";
    case admin_level::game_master:
        return "Game Master";
    case admin_level::senior_gm:
        return "Senior GM";
    case admin_level::admin:
        return "Admin";
    case admin_level::owner:
        return "Owner";
    }
    return "Unknown";
}
...
struct command_result
{
    ...
    static auto error(std::string_view msg) -> command_result { return command_result{false, std::string(msg), false}; }
};
...
{
public:
    simple_command(command_info info, command_handler handler) : info_(std::move(info)), handler_(std::move(handler)) {}

    [[nodiscard]] auto info() const -> const command_info& override { return info_; }

    [[nodiscard]] auto execute(const command_context& ctx) -> command_result override { return handler_(ctx); }
...
struct parse_result
{
    ...
    static auto error(std::string_view msg) -> parse_result { return parse_result{false, "", {}, std::string(msg)}; }
};
