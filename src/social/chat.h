...
struct chat_settings {
    ...
    [[nodiscard]] auto is_player_blocked(player_id player) const -> bool {
        return blocked_players.contains(player);
    }

    auto block_player(player_id player) -> bool {
        return blocked_players.insert(player).second;
    }

    auto unblock_player(player_id player) -> bool {
        return blocked_players.erase(player) > 0;
    }
};
...
namespace std {
template<>
struct hash<hb::social::party_id> {
    auto operator()(const hb::social::party_id& id) const noexcept -> size_t {
