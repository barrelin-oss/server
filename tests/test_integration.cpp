...
protected:
    ...
    auto route_message(connection_id conn, session_id sess, player_id player,
                       message_id msg_id, std::function<void(message_writer&)> write_body = nullptr)
        -> bool {
    ...
    [[nodiscard]] auto get_sent_message_count() const -> size_t {
...
TEST_F(integration_test, help_request_handled) {
    ...
    auto handled = route_message(
        connection_id{1}, session_id{1}, player_id{},
        message_id::command_common,
        [](message_writer& w) {
            w.write_u16(static_cast<uint16_t>(common_type::request_help));
        }
    );
...
TEST_F(integration_test, non_help_command_falls_through) {
    ...
    auto handled = route_message(
        ...
        message_id::command_common,
        [](message_writer& w) {
            w.write_u16(static_cast<uint16_t>(common_type::toggle_combat_mode));
        }
    );
...
TEST_F(integration_test, movement_requires_player) {
    ...
    auto handled = route_message(
        connection_id{1}, session_id{1}, player_id{},  // Invalid player
...
TEST_F(integration_test, movement_with_valid_player) {
    ...
    auto handled = route_message(
        connection_id{1}, session_id{1}, pid,
        message_id::command_motion,
        [](message_writer& w) {
            w.write_u8(static_cast<uint8_t>(wave3::motion_type::move));
            w.write_i16(100);  // x
...
TEST_F(integration_test, stats_track_all_waves) {
    ...
    route_message(connection_id{1}, session_id{1}, player_id{},
        message_id::command_common,
        [](message_writer& w) {
            w.write_u16(static_cast<uint16_t>(common_type::request_help));
        }
