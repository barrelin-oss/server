...
struct static_tile
{
    ...
    [[nodiscard]] constexpr auto is_walkable() const -> bool { return !has_flag(flags, tile_flags::blocks_movement); }

    [[nodiscard]] constexpr auto is_teleport() const -> bool { return has_flag(flags, tile_flags::is_teleport); }

    [[nodiscard]] constexpr auto is_water() const -> bool { return has_flag(flags, tile_flags::is_water); }

    [[nodiscard]] constexpr auto is_farm() const -> bool { return has_flag(flags, tile_flags::is_farm); }

    [[nodiscard]] constexpr auto is_safe_zone() const -> bool { return has_flag(flags, tile_flags::is_safe_zone); }
};
...
struct dynamic_tile
{
    ...
    [[nodiscard]] constexpr auto has_occupant() const -> bool { return occupant.value != 0; }

    [[nodiscard]] constexpr auto has_dead_entity() const -> bool { return dead_entity.value != 0; }

    [[nodiscard]] constexpr auto has_dynamic_object() const -> bool { return dynamic_object_id != 0; }

    [[nodiscard]] constexpr auto has_items() const -> bool { return item_count > 0; }
    ...
    void clear_dead_entity()
    {
        dead_entity = entity_id{0};
        dead_type = owner_type::none;
        flags = static_cast<dynamic_tile_flags>(static_cast<uint8_t>(flags) &
                                                ~static_cast<uint8_t>(dynamic_tile_flags::has_corpse));
    }
};
