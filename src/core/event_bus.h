...
public:
    ...
    template<typename E>
    auto subscribe(std::function<void(const E&)> handler) -> subscription_id {
    ...
    template<typename E>
    [[nodiscard]] auto subscriber_count() const -> size_t {
        std::lock_guard lock(mutex_);
...
private:
    ...
    template<typename E>
    struct handler_wrapper : handler_wrapper_base {
        std::function<void(const E&)> handler;
        subscription_id sub_id;

        handler_wrapper(std::function<void(const E&)> h, subscription_id id)
            : handler(std::move(h))
            , sub_id(id)
        {}
        ...
        [[nodiscard]] auto id() const -> subscription_id override {
            return sub_id;
        }
    };
...
public:
    ...
    subscription_guard(event_bus& bus, subscription_id id)
        : bus_(&bus)
        , id_(id)
    {}
    ...
    [[nodiscard]] auto id() const -> subscription_id {
        return id_;
    }
...
template<typename E>
[[nodiscard]] auto subscribe_scoped(event_bus& bus, std::function<void(const E&)> handler)
    -> subscription_guard
{
