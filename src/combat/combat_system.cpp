    ...
    if (result.hit.is_hit())
    {
        ...
        if (ctx.weapon_enchantment != item::enchantment_type::none)
        {
            if (player_sys2)
            {
                ...
                {
                    auto effect = process_weapon_effect(ctx.weapon_enchantment,
                                                        ctx.weapon_enchantment_value,
                                                        result.hit.final_damage,
                                                        atk_p->mp,
                                                        atk_p->computed.max_mp);
...
auto combat_system::build_combat_context(hb::entity::entity attacker,
...
void combat_system::apply_damage(hb::entity::entity target, const hit_result& result, hb::entity::entity source)
{
    ...
    LOG_DEBUG(general,
              "Entity {} dealt {} damage to entity {} (crit: {})",
              source.id,
              result.final_damage,
              target.id,
              result.is_critical());
