...
template<typename T>
class component_storage_wrapper : public component_storage_base {
public:
...
public:
    ...
    auto add_component(entity e, Args&&... args) -> T& {
    ...
    template<typename T>
    [[nodiscard]] auto get_all_with() -> component_storage<T>* {
        return get_storage<T>();
    }

    template<typename T>
    [[nodiscard]] auto get_all_with() const -> const component_storage<T>* {
        return get_storage<T>();
    }
    ...
    void for_each(Func&& func) {
        ...
        if (storage1->size() <= storage2->size()) {
            storage1->for_each([&](entity e, T1& c1) {
                if (auto* c2 = storage2->get(e)) {
                    func(e, c1, *c2);
                }
            });
        } else {
            storage2->for_each([&](entity e, T2& c2) {
...
private:
    template<typename T>
    auto get_or_create_storage() -> component_storage<T>& {
    ...
    template<typename T>
    auto get_storage() -> component_storage<T>* {
        auto type_id = std::type_index(typeid(T));
        auto it = storages_.find(type_id);
    ...
    template<typename T>
    auto get_storage() const -> const component_storage<T>* {
        auto type_id = std::type_index(typeid(T));
        auto it = storages_.find(type_id);
        if (it == storages_.end()) return nullptr;
