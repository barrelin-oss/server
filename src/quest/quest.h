...
struct quest_template {
    ...
    [[nodiscard]] auto is_timed() const -> bool {
...
struct quest_state {
    ...
    [[nodiscard]] auto is_complete() const -> bool {
        return status == quest_status::complete;
    }

    [[nodiscard]] auto is_turned_in() const -> bool {
        return status == quest_status::turned_in;
    }

    [[nodiscard]] auto is_failed() const -> bool {
        return status == quest_status::failed;
    }

    [[nodiscard]] auto is_abandoned() const -> bool {
        return status == quest_status::abandoned;
    }
    ...
    [[nodiscard]] auto progress(const std::vector<objective_template>& templates) const -> float {
        if (objectives.empty()) return status == quest_status::complete ? 1.0f : 0.0f;
    ...
    [[nodiscard]] auto has_timed_out() const -> bool {
        if (time_limit_seconds <= 0) return false;
    ...
    [[nodiscard]] auto remaining_time() const -> int32_t {
        if (time_limit_seconds <= 0) return 0;
    ...
    void initialize_objectives(const std::vector<objective_template>& templates) {
        ...
        for (const auto& tmpl : templates) {
            ...
            std::visit([&state](const auto& data) {
...
struct quest_journal {
    ...
    [[nodiscard]] auto can_accept_quest() const -> bool {
