...
using objective_data = std::variant<kill_objective_data,
...
struct objective_state
{
    ...
    [[nodiscard]] auto is_complete() const -> bool { return status == objective_status::complete; }

    [[nodiscard]] auto is_failed() const -> bool { return status == objective_status::failed; }
    ...
    {
        if (status != objective_status::incomplete)
            return false;
    ...
    // Mark as failed
    void fail() { status = objective_status::failed; }
