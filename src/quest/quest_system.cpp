...
{
    if (player_journals_.contains(id))
        return;
...
auto quest_system::accept_quest(player_id player, quest_id quest) -> accept_result
{
    ...
    LOG_DEBUG(general, "Player {} accepted quest {} ({})", player.value, quest.value, tmpl->name);
...
auto quest_system::abandon_quest(player_id player, quest_id quest) -> bool
{
    ...
    auto it = std::find_if(journal->active_quests.begin(),
                           journal->active_quests.end(),
...
auto quest_system::complete_quest(player_id player, quest_id quest) -> complete_result
{
    ...
    const auto* tmpl = get_quest_template(quest);
    if (!tmpl)
        return complete_result::quest_not_found;
    ...
    final_rewards.experience =
        static_cast<int64_t>(static_cast<float>(final_rewards.experience) * config_.quest_exp_modifier);
    final_rewards.gold = static_cast<int64_t>(static_cast<float>(final_rewards.gold) * config_.quest_gold_modifier);
    ...
    LOG_DEBUG(general, "Player {} completed quest {} ({})", player.value, quest.value, tmpl->name);
...
auto quest_system::get_available_quests(player_id player,
                                        int16_t player_level,
                                        uint8_t faction) const -> std::vector<quest_id>
{
    std::vector<quest_id> result;
    ...
    for (const auto& [id, tmpl] : quest_templates_)
    {
        ...
        if (player_level < tmpl.min_level || player_level > tmpl.max_level)
            continue;

        // Check faction (0 = any faction)
        if (tmpl.required_faction != 0 && tmpl.required_faction != faction)
            continue;
...
void quest_system::update_kill_objectives(player_id player, npc_id killed, bool was_player)
{
    ...
    for (auto& quest : journal->active_quests)
    {
        ...
        for (size_t i = 0; i < quest.objectives.size() && i < tmpl->objectives.size(); ++i)
        {
            ...
            if (obj.is_complete())
                continue;
            if (obj_tmpl.type != objective_type::kill_monster && obj_tmpl.type != objective_type::kill_player)
                continue;
...
void quest_system::update_collect_objectives(player_id player, item_id item, int32_t count)
{
    ...
    for (auto& quest : journal->active_quests)
    {
        ...
        for (size_t i = 0; i < quest.objectives.size() && i < tmpl->objectives.size(); ++i)
        {
            ...
            if (obj.is_complete())
                continue;
            if (obj_tmpl.type != objective_type::collect_item && obj_tmpl.type != objective_type::deliver_item)
                continue;
...
void quest_system::update_location_objectives(player_id player, map_id map, int16_t x, int16_t y)
{
    ...
    for (auto& quest : journal->active_quests)
    {
        ...
        for (size_t i = 0; i < quest.objectives.size() && i < tmpl->objectives.size(); ++i)
        {
            ...
            if (obj.is_complete())
                continue;
            if (obj_tmpl.type != objective_type::visit_location)
                continue;
...
void quest_system::update_npc_objectives(player_id player, npc_id npc)
{
    ...
    for (auto& quest : journal->active_quests)
    {
        ...
        for (size_t i = 0; i < quest.objectives.size() && i < tmpl->objectives.size(); ++i)
        {
            ...
            if (obj.is_complete())
                continue;
            if (obj_tmpl.type != objective_type::talk_to_npc)
                continue;
...
void quest_system::update_craft_objectives(player_id player, item_id item, int32_t count)
{
    ...
    for (auto& quest : journal->active_quests)
    {
        ...
        for (size_t i = 0; i < quest.objectives.size() && i < tmpl->objectives.size(); ++i)
        {
            ...
            if (obj.is_complete())
                continue;
            if (obj_tmpl.type != objective_type::craft_item)
                continue;
...
void quest_system::update_gather_objectives(player_id player, uint8_t skill_type, int32_t count)
{
    ...
    for (auto& quest : journal->active_quests)
    {
        ...
        for (size_t i = 0; i < quest.objectives.size() && i < tmpl->objectives.size(); ++i)
        {
            ...
            if (obj.is_complete())
                continue;
            if (obj_tmpl.type != objective_type::gather_resource)
                continue;
...
void quest_system::update_level_objectives(player_id player, int16_t level, uint8_t skill_type)
{
    ...
    for (auto& quest : journal->active_quests)
    {
        ...
        for (size_t i = 0; i < quest.objectives.size() && i < tmpl->objectives.size(); ++i)
        {
            ...
            if (obj.is_complete())
                continue;
            if (obj_tmpl.type != objective_type::reach_level && obj_tmpl.type != objective_type::reach_skill_level)
                continue;
    ...
    for (auto& [player_id, journal] : player_journals_)
    {
        for (auto& quest : journal.active_quests)
        {
            ...
            if (quest.has_timed_out())
            {
                ...
                LOG_DEBUG(general, "Player {} quest {} timed out", player_id.value, quest.template_id.value);
            }
        }
    }
}
