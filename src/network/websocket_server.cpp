...
ws_connection::ws_connection(connection_id id, ix::WebSocket* socket) : id_(id), socket_(socket) {}
...
{
    if (!socket_ || state_ == ws_connection_state::disconnected || state_ == ws_connection_state::disconnecting)
    {
        return;
    }
...
{
    if (socket_ && state_ != ws_connection_state::disconnecting && state_ != ws_connection_state::disconnected)
    {
...
{
    return socket_ && state_ != ws_connection_state::disconnected && state_ != ws_connection_state::disconnecting &&
        ...
        server_->setOnClientMessageCallback(
                   ...
                   const ix::WebSocketMessagePtr& msg)
            {
                ...
                case ix::WebSocketMessageType::Close:
                {
                    if (conn_id.is_valid())
                    {
                        ...
                        // This avoids modifying lookup maps from the callback thread
                        {
                            ...
                            pending_disconnects_.push_back({conn_id, msg->closeInfo.reason, state_ptr});
                        }
                    }
                    break;
                }
                ...
                case ix::WebSocketMessageType::Message:
                {
                    ...
                    {
                        LOG_WARN(network, "Failed to parse message from {}: {}", conn_id.value, parse_result.error());
                ...
                {
                    LOG_ERROR(network, "WebSocket error for connection {}: {}", conn_id.value, msg->errorInfo.reason);
...
void websocket_server::broadcast_to_authenticated(const json_message& msg)
{
    ...
    {
        if (conn->is_open() &&
            (conn->state() == ws_connection_state::authenticated || conn->state() == ws_connection_state::in_game))
        {
            conn->send_raw(json_str);
        }
    }
...
auto websocket_server::get_admin_subscribers(map_id map) -> std::vector<connection_id>
{
    ...
    for (const auto& [id, conn] : connections_)
    {
        if (conn->state() != ws_connection_state::admin_dashboard)
            continue;
...
auto websocket_server::authenticated_count() const -> size_t
{
    ...
    for (const auto& [id, conn] : connections_)
    {
        if (conn->state() == ws_connection_state::authenticated || conn->state() == ws_connection_state::in_game)
        {
            ++count;
        }
    }
