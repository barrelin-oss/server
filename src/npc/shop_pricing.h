...
calculate_sell_price_equipment(int32_t base_price, int16_t cur_dur, int16_t max_dur, bool is_neutral_map) -> int32_t
{
    if (base_price <= 0 || max_dur <= 0 || cur_dur <= 0)
        return 0;

    int32_t price = static_cast<int32_t>(static_cast<double>(cur_dur) / max_dur * 0.5 * base_price);
    if (is_neutral_map)
        price /= 2;
...
[[nodiscard]] inline auto
calculate_sell_price_consumable(int32_t base_price, int16_t count, bool is_neutral_map) -> int32_t
{
    if (base_price <= 0 || count <= 0)
        return 0;
...
{
    if (base_price <= 0 || max_dur <= 0)
        return 0;
    if (cur_dur >= max_dur)
        return 0; // Already fully repaired
...
{
    return std::find(shop.buy_categories.begin(), shop.buy_categories.end(), category) != shop.buy_categories.end();
}
...
[[nodiscard]] inline auto can_buy_in_territory(faction player_faction, std::string_view map_location) -> bool
{
    if (player_faction == faction::neutral)
        return true;
    ...
    bool is_elvine_territory =
        map_location.find("elvine") != std::string_view::npos || map_location.find("Elvine") != std::string_view::npos;

    if (is_aresden_territory && player_faction != faction::aresden)
        return false;
    if (is_elvine_territory && player_faction != faction::elvine)
        return false;
