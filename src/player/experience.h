...
struct experience_state {
    ...
    [[nodiscard]] auto exp_to_next_level() const -> int64_t {
        return exp_table.exp_to_next(level, experience);
    }
    ...
    [[nodiscard]] auto exp_progress() const -> float {
        ...
        if (level_range <= 0) return 1.0f;
        return static_cast<float>(experience - current_level_exp) / level_range;
    }

    [[nodiscard]] auto ek_contribution() const -> int32_t {
        return enemy_kill_count * contribution_per_ek;
    }
