...
auto connection::process_send() -> bool {
    ...
    std::span<const uint8_t> remaining{
        current_send_.data() + send_offset_,
        current_send_.size() - send_offset_
    };
    ...
    if (result.is_err()) {
        ...
        // Connection error
        LOG_DEBUG(network, "Connection {} send error: {}",
            id_.value, error_string(result.error()));
        return false;
    }
...
auto connection::process_receive() -> bool {
    ...
    if (result.is_err()) {
        ...
        LOG_DEBUG(network, "Connection {} receive error: {}",
            id_.value, error_string(result.error()));
        return false;
    }
