...
TEST(admin_web_protocol, admin_response_generic_success)
{
    ...
    auto msg = network::make_admin_response(network::json_message_type::admin_server_stats_response, 1, true, data);
...
TEST(admin_web_protocol, admin_response_generic_failure)
{
    auto msg = network::make_admin_response(
        network::json_message_type::admin_get_player_response, 2, false, {}, "Player not found");
    EXPECT_FALSE(msg.data["success"].get<bool>());
...
{
    nlohmann::json j = {{"player_name", "BadPlayer"}, {"reason", "Hacking"}, {"duration_hours", 24}};
...
{
    nlohmann::json j = {{"player_name", "Player1"}, {"dest_map", "elvine"}, {"dest_x", 50}, {"dest_y", 60}};
...
{
    nlohmann::json j = {{"player_name", "Player1"}, {"modifications", {{"hp", 500}, {"level", 50}}}};
...
{
    nlohmann::json j = {{"npc_name", "Slime"}, {"map_name", "aresden"}, {"x", 30}, {"y", 40}, {"count", 5}};
...
{
    nlohmann::json j = {{"player_name", "Player1"}, {"item_template_id", 77}, {"count", 10}};
...
{
    auto msg = network::make_admin_response(network::json_message_type::admin_list_players_response,
                                            99,
                                            true,
                                            {{"count", 3}, {"players", nlohmann::json::array()}});
...
{
    auto msg = network::make_admin_response(
        network::json_message_type::admin_broadcast_response, 50, true, {{"message", "Hello"}});
    auto json_str = msg.to_json().dump();
    auto parse_result = network::json_message::parse(json_str);
    ASSERT_TRUE(parse_result.is_ok());
...
{
    auto msg = network::make_admin_response(network::json_message_type::admin_mute_player_response,
                                            51,
                                            true,
                                            {{"player_name", "Test"}, {"duration_minutes", 30}});
    auto json_str = msg.to_json().dump();
    auto parse_result = network::json_message::parse(json_str);
    ASSERT_TRUE(parse_result.is_ok());
...
TEST(admin_web_protocol, search_players_response_roundtrip)
{
    nlohmann::json data = {{"success", true},
                           {"query", "zer"},
                           {"count", 1},
                           {"players", nlohmann::json::array({{{"id", 1}, {"name", "zero"}, {"level", 100}}})}};
    auto msg = network::make_admin_response(network::json_message_type::admin_search_players_response, 52, true, data);
    auto json_str = msg.to_json().dump();
    auto parse_result = network::json_message::parse(json_str);
    ASSERT_TRUE(parse_result.is_ok());
...
{
    nlohmann::json j = {
        {"guild_name", "TestGuild"}, {"action", "set_rank"}, {"target_player", "Player1"}, {"rank", "officer"}};
    auto result = network::admin_guild_action_request_data::from_json(j);
    ASSERT_TRUE(result.is_ok());
...
TEST(admin_web_config, apply_dot_values_basic)
{
    ...
    nlohmann::json values = {
        {"database.host", "newhost"}, {"websocket.port", 3000}, {"auth.allow_registration", false}};
...
TEST(admin_web_config, apply_dot_values_skips_sentinel)
{
    ...
    nlohmann::json values = {{"database.password", "***"}, {"database.host", "updated"}};
    auto result = cfg.apply_dot_values(values);
    ASSERT_TRUE(result.is_ok());
...
{
    nlohmann::json j = {{"player_name", "zero"}, {"action", "set"}, {"skill_type", 5}, {"value", 100}};
...
{
    nlohmann::json j = {{"player_name", "zero"}, {"action", "learn"}, {"spell_id", 10}};
...
{
    nlohmann::json j = {{"player_name", "zero"}, {"action", "complete"}, {"quest_id", 5}};
...
{
    nlohmann::json j = {{"player_name", "zero"}, {"mode", "all"}};
...
{
    nlohmann::json j = {{"player_name", "zero"}, {"mode", "group"}, {"group", 12}};
    auto r = network::admin_remove_effects_request_data::from_json(j);
    ASSERT_TRUE(r.is_ok());
...
{
    nlohmann::json j = {{"username", "newplayer"}, {"password", "pass123"}};
...
{
    nlohmann::json j = {{"username", "admin"}, {"password", "pass"}, {"admin_level", 20}};
    auto r = network::admin_create_account_request_data::from_json(j);
    ASSERT_TRUE(r.is_ok());
...
{
    nlohmann::json j = {{"username", "player1"}, {"new_password", "newpass"}};
...
{
    nlohmann::json j = {{"username", "player1"}, {"admin_level", 10}};
...
{
    nlohmann::json j = {{"enabled", true}, {"message", "Server maintenance"}};
...
{
    nlohmann::json j = {{"username", "player1"},
                        {"name", "NewChar"},
                        {"gender", 1},
                        {"hair_style", 2},
                        {"hair_color", 3},
...
{
    nlohmann::json j = {{"username", "player1"}, {"character_name", "OldChar"}};
...
{
    nlohmann::json j = {{"action", "add"}, {"ip", "1.2.3.4"}, {"reason", "spam"}};
    auto r = network::admin_manage_ip_bans_request_data::from_json(j);
    ASSERT_TRUE(r.is_ok());
...
TEST(admin_web_task_registry, scheduler_register_and_query)
{
    ...
    int def_count = 0;
    sched.for_each_definition(
        [&](const hb::scheduler::task_definition& def, bool running)
        {
...
TEST(admin_web_task_registry, scheduler_cancel_and_restart)
{
    ...
    int call_count = 0;
    sched.register_task("restartable",
                        "A restartable task",
                        hb::duration_ms{100},
                        true,
                        [&]() -> hb::task_callback
...
TEST(admin_web_task_registry, scheduler_start_with_override_interval)
{
    ...
    sched.register_task("custom_interval",
                        "desc",
                        hb::duration_ms{1000},
                        true,
                        []() -> hb::task_callback
    ...
    bool found = false;
    sched.for_each_task(
        [&](const hb::scheduler::task_info& info)
        {
