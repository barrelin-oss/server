...
auto get_last_error() -> socket_error
{
    ...
    {
    case WSAEWOULDBLOCK:
        return socket_error::would_block;
    case WSAECONNRESET:
        return socket_error::connection_reset;
    case WSAECONNREFUSED:
        return socket_error::connection_refused;
    case WSAENOTCONN:
        return socket_error::not_connected;
    case WSAEADDRINUSE:
        return socket_error::address_in_use;
    case WSAEINVAL:
        return socket_error::invalid_argument;
    case 0:
...
auto error_string(socket_error err) -> std::string
{
    switch (err)
    {
    case socket_error::none:
        return "No error";
    case socket_error::would_block:
        return "Operation would block";
    case socket_error::connection_reset:
        return "Connection reset by peer";
    case socket_error::connection_refused:
        return "Connection refused";
    case socket_error::not_connected:
        return "Socket not connected";
    case socket_error::address_in_use:
        return "Address already in use";
    case socket_error::invalid_argument:
        return "Invalid argument";
    case socket_error::unknown:
...
auto initialize_sockets() -> result<void, std::string>
{
    ...
    if (!wsa_initialized)
    {
        ...
        if (wsa_result != 0)
        {
            return result<void, std::string>::err("WSAStartup failed with error: " + std::to_string(wsa_result));
        }
...
tcp_socket::tcp_socket(socket_handle handle) : handle_(handle) {}
...
tcp_socket::tcp_socket(tcp_socket&& other) noexcept : handle_(other.handle_)
{
...
auto tcp_socket::accept() -> result<std::pair<tcp_socket, socket_address>, socket_error>
{
    if (!is_valid())
    {
        return result<std::pair<tcp_socket, socket_address>, socket_error>::err(socket_error::invalid_argument);
    }

    sockaddr_in client_addr{};
    socklen_t addr_len = sizeof(client_addr);

    socket_handle client_handle = ::accept(handle_, reinterpret_cast<sockaddr*>(&client_addr), &addr_len);

    if (client_handle == invalid_socket_handle)
    {
        return result<std::pair<tcp_socket, socket_address>, socket_error>::err(get_last_error());
    }
...
auto tcp_socket::set_no_delay(bool no_delay) -> result<void, socket_error>
{
    ...
    if (setsockopt(handle_, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const char*>(&opt), sizeof(opt)) != 0)
    {
        return result<void, socket_error>::err(get_last_error());
    }
...
auto tcp_socket::set_keep_alive(bool keep_alive) -> result<void, socket_error>
{
    ...
    if (setsockopt(handle_, SOL_SOCKET, SO_KEEPALIVE, reinterpret_cast<const char*>(&opt), sizeof(opt)) != 0)
    {
        return result<void, socket_error>::err(get_last_error());
    }
...
auto tcp_listener::start(uint16_t port) -> result<void, std::string>
{
    ...
    {
        return result<void, std::string>::err("Failed to create socket: " + error_string(create_result.error()));
    }
    ...
    {
        return result<void, std::string>::err("Failed to bind to port " + std::to_string(port) + ": " +
    ...
    {
        return result<void, std::string>::err("Failed to listen: " + error_string(listen_result.error()));
    }
