...
auto handle_join_party(const handler_context& ctx) -> handle_result {
    ...
    LOG_DEBUG(proto_bridge, "Join party: player={} target={}",
              ctx.player.value, target_player.value);
...
auto handle_party_response(const handler_context& ctx, bool accept) -> handle_result {
    ...
    LOG_DEBUG(proto_bridge, "Party response: player={} party={} accept={}",
              ctx.player.value, party_id_val.value, accept ? "true" : "false");
...
auto handle_create_guild(const handler_context& ctx) -> handle_result {
    ...
    LOG_DEBUG(proto_bridge, "Create guild: player={} name='{}' tag='{}'",
              ctx.player.value, guild_name, guild_tag);
...
auto handle_guild_join_response(const handler_context& ctx, bool approve) -> handle_result {
    ...
    LOG_DEBUG(proto_bridge, "Guild join response: approver={} target={} approve={}",
              ctx.player.value, target_player.value, approve ? "true" : "false");
...
auto handle_guild_dismiss(const handler_context& ctx) -> handle_result {
    ...
    LOG_DEBUG(proto_bridge, "Guild dismiss: kicker={} target={}",
              ctx.player.value, target_player.value);
...
auto handle_exchange_request(const handler_context& ctx) -> handle_result {
    ...
    LOG_DEBUG(proto_bridge, "Exchange request: player={} target={}",
              ctx.player.value, target_player.value);
...
auto handle_set_exchange_item(const handler_context& ctx) -> handle_result {
    ...
    LOG_DEBUG(proto_bridge, "Set exchange item: player={} item={}",
              ctx.player.value, item_id_val.value);
...
void send_guild_result(const handler_context& ctx, bool success, std::string_view message) {
    ...
    if (success) {
        send_response(ctx, protocol::message_id::response_create_new_guild, [&](protocol::message_writer& writer) {
