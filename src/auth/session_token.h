...
struct session_token
{
    ...
    [[nodiscard]] auto is_expired() const -> bool { return std::chrono::system_clock::now() >= expires_at; }
...
[[nodiscard]] auto generate_token(const session_token_config& config) -> result<std::string, std::string>;
...
[[nodiscard]] auto
create_session_token(account_id account,
                     std::chrono::seconds duration = std::chrono::seconds{3600},
                     std::optional<std::string_view> ip_address = std::nullopt,
                     std::optional<std::string_view> user_agent = std::nullopt) -> result<session_token, std::string>;
