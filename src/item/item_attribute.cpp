...
auto item_attribute::to_json() const -> nlohmann::json
{
    ...
    if (upgrade_level > 0) j["upgrade"] = upgrade_level;
    ...
    if (custom_made) {
        j["cm"] = true;
        if (custom_quality != 0) j["cq"] = custom_quality;
    }
    return j;
}
...
{
    item_attribute attr;
    if (j.is_null() || !j.is_object()) return attr;

    if (j.contains("upgrade")) attr.upgrade_level = std::min<uint8_t>(j["upgrade"].get<uint8_t>(), 15);
    if (j.contains("mt")) {
        attr.main_type = static_cast<enchantment_type>(j["mt"].get<uint8_t>());
        if (j.contains("mv")) attr.main_value = std::min<uint8_t>(j["mv"].get<uint8_t>(), 15);
    }
    if (j.contains("st")) {
        attr.sub_type = static_cast<sub_enchantment_type>(j["st"].get<uint8_t>());
        if (j.contains("sv")) attr.sub_value = std::min<uint8_t>(j["sv"].get<uint8_t>(), 15);
    }
    if (j.contains("cm")) {
        attr.custom_made = j["cm"].get<bool>();
        if (j.contains("cq")) attr.custom_quality = std::clamp<int8_t>(j["cq"].get<int8_t>(), -100, 100);
    }
    return attr;
}
...
auto to_string(enchantment_type type) -> const char*
{
    switch (type) {
        ...
        case enchantment_type::fire: return "fire";
