...
auto alchemy_system::get_available_recipes(entity_id player) -> std::vector<const craft_recipe*>
{
    std::vector<const craft_recipe*> available;
    if (!recipes_ || !skills_)
        return available;

    auto skill_level = skills_->get_skill_level(player_id{player.value}, skill::skill_type::alchemy);
...
auto alchemy_system::attempt_craft(entity_id player, int32_t recipe_id) -> craft_result
{
    ...
    // Roll for success
    int32_t chance = calculate_success_chance(skill_level, plr->base.intelligence, *recipe);
...
auto alchemy_system::check_ingredients(entity_id player, const craft_recipe& recipe) const -> bool
{
...
void alchemy_system::consume_ingredients(entity_id player, const craft_recipe& recipe)
{
    std::unordered_map<int32_t, int32_t> needed;
    for (const auto& ing : recipe.ingredients)
    {
